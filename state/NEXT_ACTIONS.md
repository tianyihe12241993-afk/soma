...
## TOP OF QUEUE (2026-06-30 ~18:25Z) — ✅ DQ LANDED → 23.8% REAL. 3 final shots UPLOADED + queued + watcher-tracked. DEFEND + wait for them to score.
- [ ] ★★★ **3 FINAL SHOTS LIVE (in queue) — `make reward` at scored, read ONLY at scored.** Both kings now `failed review` on the board (DQ applied; watcher caught it 12:08Z). 23.8% is REAL (np2 Pair(E,M) 0.951 + Single-M 0.849; np3 Pair(M,H) 0.599). Uploaded (USER, OWN hotkeys, can't hurt floor): **cap32+pin `5DAbJikTa2…`, np3+pin `5E4Y4jzYKY…`, np2+pin `5FLUziw5Cp…`** (registered uphardsaliencesub/np3pinsub/np2pinsub; all Codex-GO). Watcher (`com.soma.compwatch`) alerts → `state/comp_watch.md` + macOS notify when any reaches scored OR fails review. AT SCORED: `python scripts/vet_draw.py --hotkey HK` → E/M/H vs np2/np3 + **Overall vs 0.703** (5DCnA57). Best = cap32+pin (Overall ~0.74-0.76 if Hard lands ~0.37). **DEFEND np2/np3/m26 — the 23.8% rests on them; the 3 shots only ADD.**
