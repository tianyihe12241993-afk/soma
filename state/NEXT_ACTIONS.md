...
_Mode: 🏆 comp-108 WON (2026-07-06). comp-109 (CoT-Compression-5) is now the ACTIVE comp. Files = source of truth. Full status: state/CURRENT.md TOP._

## TOP OF QUEUE (2026-07-06) — 🏆 WON comp-108 (cap32+pin Overall crown, ~66.6%). NEXT: comp-109 title defense + wallet ops.
- [x] ✅ **WON comp-108 (CoT-Compression-4).** cap32+pin (`5DAbJik`) = platform "Highest score in this competition" 0.723 (E1.044/H0.281/M0.862, scored). All 23 higher-raw miners = failed review (every cheater DQ'd). **Reward: Overall 57.1% (cap32+pin) + Pair(M,H) 9.5% (np3) = ~66.6% of the pool.** Screenshot: `data/raw/readme_prompting/comp108_WON_5DAbJik_2026-07-06.png`.
- [ ] ★★★ **comp-109 (CoT-Compression-5) TITLE DEFENSE — uploads until 13 Jul 14:30 UTC, eval → 20 Jul, ~41,328 dTAO pool.** `cap32+pin` (`upload_miner_uphard_salience.py`, sha256 **695b4fe4**) = the PROVEN, thrice-verified-compliant, title-winning base to re-submit. ⚠️ **Confirm the comp-109 rules first** (README_prompting.md may change between comps — run `python scripts/check_readme_current.py`; it was 429-rate-limited 2026-07-06, retry). Check comp-109's task set + whether the WEIGHTED-TOKEN scoring regime carries over. USER decides upload (own hotkey, DeepInfra+Venice routing).
- [ ] ★★ **TOOLING — scraper broke on the new dashboard.** The dashboard is now JS-loaded (no inline leaderboard JSON); archives are at `?comp=NNN` (e.g. `?comp=108`). `scripts/collect_dashboard.py` + `collect_runs.py` need updating to parse the new page (or scrape via the JS-rendered view / an API endpoint) before they'll read comp-109. Until then, use `/browse` to read the board manually.
