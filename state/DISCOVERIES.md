...
## 2026-07-07 — ★★★★★ LIVE EVAL — pipeline validated; miner fires; huge step-variance. ⚠️ VERDICT CORRECTED BELOW: 20% IS achievable (a dashboard qualifier proves it); my "cannot clear" call was a MEASUREMENT ERROR.
> **CORRECTION (later same day):** comp-110 board has a qualifier `5DtYjEJbjzi6…` (1/46) that cleared the screener with **19.8% RAW-token savings**, passing correctness, output low/stable. My "cannot clear 20%" was wrong: I measured WEIGHTED vs a fresh PAIRED baseline (2–4× variance; platform uses a FIXED seeded baseline over 15 runs), and the dashboard-comparable RAW metric puts us at 11.0% vs their 19.8%. ⇒ the gate is a compression-AGGRESSIVENESS + trajectory-STABILITY problem (compress ~2× harder, keep output/steps low), NOT a wall. Details: state/CURRENT.md LATEST-2 + reports/comp110_local_eval_findings.md correction block. The bullets below (3.5% weighted, no systematic step reduction) are the raw record of the FLAWED-baseline measurement, retained for the pipeline/variance findings which still hold.
- **Pipeline VALIDATED (first time ever):** the comp-110 copilot stack runs e2e locally; miner FIRES (36 `compress_messages` calls; sidecar out/in 0.867 = 13.3% byte reduction); solve `completed` + real patch; 100% patch rate both sides. Candidate = skeleton_v2 (sha 429d1b26), `target` profile.
- **SCREENER MEASURED:** n=6 pairs (django-14017 + django-11099 × 3 runs), ratio-of-sums (the screener's own metric) = **3.5% weighted savings** (gate 20%). Per-instance 1.67% / 9.69%. **Codex independently re-derived 3.51% — MATCH.**
- **ROOT CAUSE:** weighted tokens are dominated by `agent_steps`, which swings ~2–4× run-to-run (SAME miner: 16→66 steps on django-14017). Byte compression is real (~13–20% at EQUAL steps — run3 was 31 vs 33 steps → +20.4%) but SWAMPED by step variance, and compression does NOT systematically cut steps (miner mean 26.7 vs baseline 27.7). ⇒ the trajectory/output lever (the ONLY math path to 20%, re-derivation §5) is NOISE, not a reliable positive.
- **CONSTANTS VERIFIED** from `git show upstream/main:mcp_platform/app/core/config.py` @ 9c1be25: gate **0.2**, cache **0.1**, input 1.0, output 3.0, pass 0.5, screener tasks 3. ⚠ A STALE LOCAL `mcp_platform` copy shows old 0.1/0.333 — do NOT trust it; upstream is authoritative. (Under old cache 1/3 the same data = 8.52%, still below even a 10% gate.)
- **VERDICT (both models, both gates):** compliant pure context compression CANNOT clear the current 20% screener; profile tuning won't cross it. Reaching 20% needs ~107k more weighted ≈ **67% of output tokens** = a behavioral/step-reduction win. **Correctness (≥50% pass) NOT gradeable locally** (no per-repo swebench eval images). Full data + options: reports/comp110_local_eval_findings.md.
...
## 2026-07-08 — ★★ PR #176 RULING CONFIRMED (team statement + verified vs live repo): NOT merging for comp-110.
Team (Discord, user-relayed): PR #176 won't merge this comp — "all changes must be made before the upload window starts"; they'll announce during THIS round's eval period whether it merges for the NEXT round. VERIFIED: PR state OPEN/mergedAt=null; live README allowed-set = 15 strings UNCHANGED vs baseline; frozen f1km compliant. ⇒ (a) marker rules FROZEN for comp-110 — the PR#176 one-line switch in our candidates stays dormant; (b) ⚠️ STRATEGIC COROLLARY: the same freeze policy implies the SCORING FORMULA is also frozen for comp-110 — the "formula is changing" expectation applies to comp-111, NOT mid-comp. The 20% gate is final for THIS comp: either a ≥20% design exists under current rules, or comp-110 is a pass and everything we built (emulator, doctrine, frozen Gate-A-perfect f1km line) is comp-111 preparation. Watch the eval-period announcement for the PR#176/formula decisions.

## 2026-07-08 — ★★★★★ TOOL-TAXONOMY RE-ANALYSIS (78 runs, 63.9M-char stream): tool-aware selective compression ceiling = **0.4%** — Gate B is MATHEMATICALLY CLOSED to content compression. STOP.
- **Stream anatomy (tool-name-accurate, tool_call_id-mapped):** tool content = 31.9% of stream. Within it: file-read 64.5% (=20.6% of stream, UNTOUCHABLE—proven), search/listing 23.1% (=7.4%, mostly PATHS=must preserve), bash-command 6.4% (=2.1%, median 96 chars), test-output 0.7%, traceback 0.4%, install 0.1%. The remaining 68% of stream = system/user/assistant-protected.
- **The bash/test/log lever = ~2.4% of stream — NOTHING.** Safe ceiling (file-reads verbatim + salient-core on all command/search output): **0.4% aggregate cut**. Direction A DEAD.
- **Trajectory mining:** corr(re-reads,steps)=+0.44; high-step runs (≥30) re-read 5.0×/run vs 1.7 low-step; output/step ~constant (269-291) ⇒ output reduction ≡ step reduction. Baselines already re-read 2.9×/run (range-views). Our compression adds +0.6 re-reads/run.
- **Qualifier arithmetic decoded:** each turn re-sends the 23k system prompt; turns ∝ re-send mass. 19.8% raw savings ≈ ~25-30% fewer turns — TRAJECTORY EFFICIENCY, not content compression. No reliable compress_messages mechanism produces this (our fidelity/gentle/marker experiments all step-neutral or worse).
- **DECISION (stop/go): STOP.** No new miner for comp-110's screener. Content levers exhausted (empirically n=25 AND now structurally). Posture: hold frozen Gate-A-perfect line (f1km bd352453), watch eval-period announcements (PR#176 + formula for comp-111), all infrastructure is comp-111 prep.

## 2026-07-08 — FULL RULES RE-READ (miner/README_prompting.md, all 115 lines) — evidence-window CONFIRMED compliant + 2 corrections.
(a) Stubbing interior tool RESULTS to an approved marker = canonical omission-marker use (§1 L30-31: markers wrap omitted content; source-line reference keeps it locatable — our stub carries the full range). Preserve-clauses bind INSTRUCTIONS (system/user/tool-policy), untouched. (b) CORRECTION: [[Omitted]]/[[/Omitted]] + [[deleted]]/[[/deleted]] are ALREADY allowed (§5.1) — PR#176 concerns only the composite range-template wording; our "Omitted-pending" notes were stale. (c) UNEXPLORED COMPLIANT LEVER: §2 loop-detection guards ("fail fast with a clear loop reason", allowed reasons: repeated assistant response / repeated tool call signature) = legal mechanism to CLIP FLAIL TAILS (the −104% runs; high-step runs re-read 5×/run). Parked as evidence-window v2 addition if emulator shows flails.

## 2026-07-08 — ★★★★★ QUALIFIER FOOTPRINTS DECODED (detail collector FIXED). Both qualifiers are STRONG compressors — the ~11% low-raw hypothesis is DEAD (that number was our own broken m1).
- Detail collector FIXED: dashboard detail page is client-rendered; data streams via GET {page}?_rsc=<token> with RSC:1 + Next-Router-State-Tree headers. New tool: scripts/collect_miner_detail_rsc.py (auto-discovers _rsc token; parses task array). Old collect_miner_detail.py DEPRECATED for comp-110.
- **Qualifier #1 (5DtYjEJ): 30.2% RAW savings** over the 5 screener tasks (per-task 35/-1.5/44/8/36; in-frac 3.0%; pass_with 5/5). [The old "19.8%" was a stale 15-row scrape w/ dup attempt-groups.]
- **Qualifier #2 (5GxLcC…QRCiVN, exact hk): 25.5% RAW** (per-task 31/18/45/-1.6/-9.3; in-frac 3.0%; pass_with 5/5). ⚠ I'd been using a WRONG hotkey tail for #2 all session — corrected.
- **PATTERN (both):** input-fraction 3.0% (cache-STABLE, no cache-bust), 5/5 correctness, 25-30% raw, savings concentrated on the 3 BIG tasks (15103/11551/13964 ~30-45%), small tasks allowed to go slightly negative. This is EXACTLY our cap-ladder philosophy but ~2x more raw savings while staying cache-stable + non-breaking.
- **IMPLICATION:** the gate needs ~25-30% RAW (not the 11% I mis-inferred). Our f1km honest raw ~14-17% is genuinely SHORT — but the qualifiers PROVE the target is reachable with a NON-breaking cache-stable compressor. Redesign target is now CONCRETE: match their profile (big-task 30-45% cut, in-frac 3%, 5/5 pass). Gap = they compress big file-reads ~2x harder than f1km WITHOUT breaking. HOW is the open question (they keep pass_w/o=pass_w=True at 44% cut on 11551 — the task f1km flails on).

## 2026-07-08 — ★★★★ django-11551 REVERSE-ENGINEERING: f1km's salience defect QUANTIFIED; path to qualifier zone is PLAUSIBLE but re-read-gated.
- Ground truth 11551: fix = 21 lines in ONE function (_check_list_display_item) of admin/checks.py; FAIL_TO_PASS = test_valid_field_accessible_via_instance. Must-keep mask is TINY (0-9 lines/read).
- **f1km DEFECT CONFIRMED:** on the big code reads (12561-char django files = the dominant repeated read) f1km cuts **0%** (8.9% on a 14303 read) → **2% aggregate**. Cause: f1km's salience regex matches nearly every code line (def/import/path everywhere) → net-reduction guard returns original. THIS is why f1km caps ~14-17% raw — it barely touches the reads where savings must come from.
- **Headroom real but narrow:** TIGHT structure-only salience → 71% aggregate cut, but LOSES must-keep on ~13% of reads AND 23 markers/read (marker-menu = proven re-read trigger). MODERATE (floor 2500, contiguous drops min-run 6) → 36.6% agg, clean 19.6% + 3 markers + must-keep-OK on the dominant django code read, BUT must-keep fails on 236/401 reads (drops traceback frames/mixed content).
- **FUNDAMENTAL TENSION:** hitting 30%+ requires dropping code BODIES; the fix lives in a body; a compliant miner can't know WHICH body (task-awareness banned); so generic body-dropping risks the fix-relevant body → re-read/break. Oracle must-keep survival ≠ real safety.
- **PROPOSED capladder-v2:** top-level-structure-only salience (drop indented bodies/comments/docstrings as CONTIGUOUS runs → few markers), larger floor ~2000-2500, small/search results + fresh read untouched. Est ~20-25% on big code reads → aggregate raw maybe ~18-24% (f1km 14-17 → toward qualifier floor 25). PLAUSIBLE not proven; re-read/break risk unmeasurable offline → REQUIRES one emulator pass. No upload.

## 2026-07-08 — ★★★★★ 5 QUALIFIERS now (board 85). Footprint band CONFIRMED + threshold pinned.
All 5 qualifier raw-savings (5 screener tasks each, vs same base 18,497,737):
- 5EPDbSXL 44.9% (in-frac 5.5%, 0/5 neg, E1.22/M1.10/H1.31 = ALL POSITIVE, tot 1.211 = #1)
- 5DtYjEJ  30.2% (3.0%, 1/5 neg)
- 5GxLcC   25.5% (3.0%, 2/5)
- 5EAEY117 24.6% (2.9%, 2/5)
- 5EkKL1jD 24.2% (3.1%, 1/5)  E-4.00 (Easy floored) but qualified — savings+M+H carry
**THRESHOLD PINNED: min qualifier raw ≈ 24.2% (weighted gate 20% lands here). in-frac 2.9-5.5% (cache-stable). ALL pass_with=5/5 (screener needs correctness). Higher raw → higher total (44.9%→1.21).**
IMPLICATION for capladder-v2: our offline view-read cut 20-51% + grep-preserve is the RIGHT shape (matches 5EPDbSXL profile: aggressive but non-breaking, modest in-frac). Target is now a NUMBER: aggregate raw >=~25% with 5/5 pass. Emulator (running) tells us if v2 hits it live. NOTE 5EkKL1jD proves E can be fully sacrificed if M/H+savings carry — de-risks aggression on Easy tasks.

## 2026-07-08 — ★★★★★ LOOP-GUARD STUDY + BASELINE CORRECTION: cl2's "flail" was a BASELINE-SET ARTIFACT. Corrected cl2 = **13.3% weighted / 21.4% RAW, 10/10, estScore 1.188 — CAMPAIGN BEST, 3pts from the qualifier floor.**
- Loop-guard study (13964 flails + historical): NO collapsible loops — repeated calls were tiny docker/env probes ("No module named asgiref" = env fighting), damage = trajectory length, not duplicated evidence. Guard as compression-of-repeats has ~0 harvest. NO-GO on a loop-guard candidate (nothing to build).
