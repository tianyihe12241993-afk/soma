# CURRENT — live status (2026-07-08) — READ FIRST after compaction

## 📌 PINNED — comp-110 DOCTRINE (screener requirements + crown strategy; verified from upstream 9c1be25 + platform. Keep at top; update only on formula change.)
**SCREENER (the ticket; BOTH gates in one shot, ONE upload per hotkey):**
- **Gate A correctness:** pass ≥50% of screener tasks; per task a MAJORITY of the 3 attempts must be RESOLVED (graded, not "patch produced"). Screener = the 5 django tasks (15103/13964/11551/15375/13516), swebench_verified only.
- **Gate B savings:** ≥20% aggregate WEIGHTED savings vs the platform's FIXED seeded baseline, ratio-of-sums over all (task,attempt); `weighted = 1·input + 0.1·cached + 3·output`.
- Prereqs: OpenRouter key bound (DeepSeek provider + Data Collection ON), no missing run rows, compliant code.
- **Reality: 1/50 passed. Gate A (breaks, −4) is the killer; A and B pull opposite ways — the game is the OVERLAP.** Light-touch ≈ A-safe/B-short (~10% wt); skeleton-deep ≈ B-capable (~20% wt)/A-uncertain (250 broke Medium).
**CROWN (eval 13→20 Jul: HIDDEN tasks, 3 types × 50 × 5 runs; 7-element layers over task types — triple ≈57%, pairs ~9.5%, singles ~4.8%):**
1. QUALIFY (only screener-passers are evaluated — being one of 2–5 qualifiers beats being brilliant).
2. SURVIVE REVIEW (comp-108 lesson: all 23 higher scorers DQ'd; compliance won the crown). Approved markers only, no steering, no instruction edits.
3. DON'T BREAK (base {pp +1, break −4, flip +2}: one break erases four passes).
4. SCORE ALL 3 TYPES (triple element uses miner TOTAL; explore: PASSTHROUGH=0 — savings required; quality = hit−noise file rate; local scorer: scripts/score_explore_local.py).
5. GENERALIZE (hidden ≠ the 5 screener tasks; test diverse tasks locally).
6. TRAJECTORY EFFICIENCY (vs fixed baseline, shorter runs = free savings + less break exposure — the qualifier's edge).
**One line: be the compliant, break-free qualifier and let review + the −4s clear the field (the comp-108 playbook).**
**⚠️ ONE-UPLOAD IMPLICATION (2026-07-08): the screener-passing miner IS the eval miner — no re-upload after qualifying. The winner must clear the screener AND hold up on explore (savings>0 for tau + hit−noise quality) and edit. Emulator phase 2 includes explore/edit validation for finalists BEFORE the upload decision.**

## ★★★ 2026-07-08 (LATEST-8, DIRECTION RECONFIRMED) — full design rethink vs comp-108 champion + 2 platform shots. LOCAL EMULATOR COMPLETE (grading WORKS: RESOLVED django-11551). ⚠️ ONE UPLOAD PER HOTKEY.
- **EVIDENCE SYNTHESIS (all four sources agree on ONE architecture):**
  (1) comp-108 champion cap32+pin = VERBATIM-below-floor (16k passthrough!) + proportional keep 60% + salience pin above — LIGHT-TOUCH interior, break rate ~11%, WON. It never skeletonized to tiny budgets.
  (2) Shot 1 recoff-gentle (budget 4k incl. FRESH): E −3.98 → fresh-read compression is lethal. (3) Shot 2 interior-ultra-250: E −1.04 (fresh-protect WORKS) but M −4.00 → interior DESTRUCTION kills Medium (agent re-reads/burns steps). (4) Qualifier 5DtYjEJ: 19.8% savings + M +0.88 + LOW output/short trajectories.
- **RECONFIRMED DESIGN:** fresh-tail FULL protection (keep) + interior = champion-style VERBATIM-BELOW-FLOOR + salience-keep-above (port of cap_tool_result), floor tuned DOWN from comp-108's 16k (reads here are ~12.6k → 16k floor = 0 savings) until savings ≈ 20–25% while Medium still PASSES. The sweet spot is EMPIRICAL → found by the emulator on the REAL screener tasks, not by uploads. Accept modest Easy sacrifice (qualifier does); Hard can't break. NOTE: savings are vs a FIXED baseline → efficient/short trajectories count toward savings too (the qualifier's visible edge).
- **EMULATOR COMPLETE:** `scripts/grade_patch_local.py` (copilot backend never grades — it only captures; our grader runs the real swebench harness on the captured patch; VALIDATED: baseline django-11551 = RESOLVED, report in outputs/baseline_seed_11551_r1/). swebench 4.1.0 in the SOMA-benchmark uv env; DOCKER_DEFAULT_PLATFORM=linux/amd64. Screener tasks KNOWN: django-15103/13964/11551/15375/13516.
- **NEXT:** (1) seed fixed baseline (3 runs × 5 screener tasks; r1 of 11551 done); (2) build interior-cap ladder (floor 1k/2k/4k verbatim-below + salience-above, fresh-protected); (3) emulator-score each (graded pass + weighted ratio-of-sums); (4) Codex red-team + audit winner; (5) USER uploads ONCE. interior-deep-380 (3aad2dfe, GO) stays HELD — the cap-ladder likely supersedes it.

## ★★★ 2026-07-08 (LATEST-7) — m1 post-mortem COMPLETE. Break cause = FRESH-read compression (recency-OFF). System-trim = NON-COMPLIANT (checked). NEXT candidate = interior-only (recency-ON) at ultra interior budget. ⚠️ SCREENING TURNAROUND ≈ 10 MIN → platform = fast oracle, iterate on m1.
- **RE-ATTRIBUTION (the key insight):** m1's −4 breaks came from compressing the FRESH read the agent acts on (recency-OFF), NOT from interior compression. Interior-only (recency-ON) is comp-108-proven break-tolerable (np2 compressed history hard, ~89% runs passed). skeleton_v2 (recency-ON) was NEVER platform-tested — the local "≈0% weighted" readings were paired-baseline noise, and the honest model says interior-only ultra ≈ **15–18% weighted** (tool ≈ 42% of cached mass × ~80% cut × ~0.55 cached share), near/under the 20% gate — platform test decides.
- **SYSTEM-TRIM HYPOTHESIS DEAD (compliance):** README_prompting lines 24–36/80: "Preserve the original instruction meaning exactly / Preserve tool policy, safety policy, role policy" + prohibited "deleting instructions" + review "No instruction was added, removed, or reordered". The 23.5k system prompt (50.7% of stream) = instructions/tool-policy → untouchable. Do NOT revisit.
- **STREAM MAP (734 requests, corrected classifier):** system 50.7% + user 6.8% + assistant ~0.9% (all tool_calls → CoT-compression is a DEAD lever in this agent) + tool ~41.6% (big items = `view` file reads in `N. ` numbered format — the load-bearing content). Only compressible-without-breaking mass = INTERIOR tool copies.
- **⚠️ OPERATIONAL GOLD: screening = ~10 min turnaround** (m1: 03:43 submit → scored ~03:55). Same-hotkey re-upload restarts pipeline → we can test ~1–2 candidates/day on m1. The platform IS the fast iteration loop local could never be.
- **NEXT:** build interior-ultra candidate (skeleton_v2 base, recency-ON, interior budget → ultra ~250–380), local gates + Codex audit, USER re-upload on m1, read screener in minutes. Then iterate aggressiveness on the platform signal.

## [2026-07-08 LATEST-6] — m1 SCORED: screener FAIL, Easy −3.98 / Medium −3.10 / Hard +1.13, total 0.388. Gentle+recency-off = BREAK-LETHAL, not safe-but-weak.
- **PLATFORM RESULT (m1 = 5CDLSZYLKoJngZ…, scored ~03:5xZ):** `screener_passed=False`, `not qualified`. **Easy ≈ −4 = the BREAK floor** (pass→fail nearly every run), Medium mostly breaks, **Hard +1.13 = some FLIPS** (baseline-fail can't break — comp-108 shape repeats: breaks lethal, Hard flips free). Detail page hides per-task rows for non-qualified (tasks=0) → no paired calibration data.
- **MECHANISM:** gentle touched only ~21% of content (3.7% aggregate) but that 21% = exactly the BIG FILE READS; a 4k-char skeleton of a 12k read drops the function the agent must edit → wrong patch → −4. **Codex's break-safety concern validated at the GENTLEST setting.** ⚠️ LESSON: local "patches produced" (6/6) says NOTHING about correctness; aggregate byte-% is the wrong safety metric — per-critical-read information survival is what matters.
- **BOARD PATTERN:** every recent submission craters the same way (Easy ≈ −4 across the board); even the sole qualifier 5DtYjEJ has Easy −0.56 (rides the break edge, majority-pass + 19.8% savings). E/M/H categories ARE shown on the comp-110 board (screener view).
- **DESIGN PIVOT (measuring now):** compress only NEVER-LOAD-BEARING content — assistant CoT + test-log/noise tool output — NEVER file reads (content-classified, position-independent = cache-safe). Ceiling being measured from captured payloads. Also: local correctness grading (--swerebench-eval on django-11551, a REAL screener task with local payloads) is now MANDATORY before any re-upload.

## ★★★ 2026-07-08 (LATEST-5) — skel_recoff_v1 UPLOADED (oro-miner/m1; board shows 5CDLSZYLKoJn in `screening` 03:43Z — confirm ss58 vs the upload log). Fable-5 fresh-eyes review: WILL LIKELY FAIL the savings gate — treat as CALIBRATION, build recoff_v2 (ultra-ish) as the real shot.
- **UPLOAD MECHANICS SOLVED:** `~/.venvs/soma` was Python 3.9 → soma_shared needs 3.10+ (`str | None`) → built **`~/.venvs/soma312`** (3.12, bittensor 10.5.0 + httpx + python-dotenv + soma_shared). Runbook updated: reports/platform_commands.md (comp-110 block). Platform URL `https://platform.thesoma.ai`; wallet `oro-miner`, hotkey `m1`. Miner runtime = container Python 3.12.13; miner verified to run identically on 3.9/3.12 (`from __future__ import annotations`) — version mismatch affected ONLY the upload tool, never scoring.
- **FRESH-EYES QUANTIFICATION (734 real captured requests through the EXACT uploaded module, sha 2d6ed767):** aggregate byte-reduction = **3.7%** (vs target's 22%). Three stacked pass-through holes: <2000-char msgs (40.9% of compressible content) + **MIN_LINES=47 hole** (big-but-few-lines, 9.9%) + **fits-in-4000-budget → all-kept verbatim (28.3%)**; only 20.9% of content is touched. ⇒ **expect `screener_passed=False`**. I over-corrected on Codex's safety change — gentle is too gentle, quantified.
- **NOT WASTED:** (a) same-hotkey re-upload allowed (restarts pipeline; window to 13 Jul); (b) after screening, scrape m1's detail (`collect_miner_detail.py --hotkey <ss58>`) → platform per-task tokens_with/without_compression = the FIRST local↔platform calibration point.
- **THE v2 MATH (recoff frontier):** recency-OFF ⇒ position-independent ⇒ no cache-bust/double-charge ⇒ weighted savings ≈ context-byte-cut × ~0.75 (input+cached weighted share). Byte-sweep: deep 25%→~19%wt, **ultra 27.4%→~20.5%wt, ultra2 29%→~21.7%wt** (qualifier's 19.8% raw sits in this band — consistent). ⇒ **recoff_v2 = recency-off + ~ultra aggressiveness + FIX MIN_LINES (decouple from head/tail) + fix stale docstring (still says recency default-on — human review reads it)**. Open risk = correctness under an aggressive fresh read → local patch-rate/steps sanity + Codex re-audit BEFORE re-upload (USER-run). Uploaded bytes frozen: experiments/candidates/RECOFF_comp110_v1/frozen/skel_recoff_v1_UPLOADED_2d6ed767.py.

## ★★★ 2026-07-08 (LATEST-4) — FRESH-INPUT candidate `skel_recoff_v1` = Codex GO-WITH-CHANGES (APPLIED) → UPLOADED (see LATEST-5).
- **THE REFRAME (broke the "compression can't win" loop):** weighted = 1·input + 0.1·cached + 3·output. Our recency guard compressed each tool result ONLY while CACHED (×0.1), NEVER while FRESH INPUT (×1.0) → weighted savings ≈ 0. **FLIP = recency OFF (compress the fresh input too).** Candidate: `miner/cot_compression/upload_miner_skel_recoff_v1.py` = skeleton_v2 + `_RECENCY=False`, default profile now `gentle`.
- **Codex gate #1: GO-WITH-CHANGES** (task b5h8kugb4; full: reports/codex_recoff_audit.md). Compliance/cache-safety/robustness PASS; diff vs GO'd skeleton_v2 = only the recency flip. ONE change requested: `target`/550 too aggressive on the fresh read (wrong-patch/re-read risk; 3/3 local patches ≠ correctness). **APPLIED:** added `gentle` profile (min2000/head30/tail15/budget4000) as the DEFAULT → keeps ~41% of a big read (12.5k→5.2k, 59% cut) vs target's ~4%; cache-stable, deterministic, compliance re-PASS.
- **WHY UPLOAD (platform-truth test, the ONLY valid measurement):** local CANNOT decide this — weighted savings is step-variance-dominated (confirmed ~20 solves + 3 offline models) and correctness is ungradeable locally (no swebench eval images). Only the platform (FIXED baseline, 15 runs, real grading) tells if recency-off clears 20% weighted + passes ≥50%. **USER-run, SPARE hotkey** (never the live winners). Read at `status=scored`: weighted savings vs 0.20 + qualified?
- **HONEST odds:** recency-off targets the RIGHT tokens (input ×1.0) + is cache-stable + hints lower per-turn output — genuinely better-founded than every prior candidate — but whether it reaches 20% is UNKNOWN until the platform test. This is the pursue-path the USER chose (formula unlikely to change soon; the dashboard qualifier 5DtYjEJ proves 20% is achievable).

## ★★★ 2026-07-07 (LATEST-3, SETTLED) — the weighted gate is an OUTPUT + CACHE-STABILITY lever, NOT byte-compression. Byte-tuning (deep/ultra) ≈ 0 weighted savings → would FAIL the screener. [supersedes LATEST-2's "22% competitive"]
- **SCREENER CODE READ** (upstream 9c1be25 `swebench_orchestrator.py:_evaluate_screening_for_script` + `_weighted_tokens_for_screening`): weighted = **1·input + 0.1·cached + 3·output**, FIXED seeded baseline (`baseline_run=True`), ratio-of-sums per (task,attempt), gate ≥0.20. Raw fallback only if splits missing (they are populated). [CODE FACT]
- **COMPOSITION (our runs + qualifier):** weighted ≈ **50–60% cached (×0.1) · 20–30% output (×3) · 15–20% input (×1)**. Byte-compressing CACHED history (our skeleton's whole lever) is 0.1-weighted → even cache-stable, a 22% cache cut = only ~12% weighted; AND our rewriting MILDLY BUSTS CACHE (input fraction **5.4% vs baseline 3.7%**) → the +input (×1.0) cancels the cached savings → **MEASURED deep≈baseline weighted at equal steps (~0%)**. Deep/ultra aggressiveness does NOT help (can worsen the bust).
- **QUALIFIER 5DtYjEJ DECODED:** input fraction **3.0%** (cache-STABLE), output/run ~6.6k (vs our ~9.8k), NORMAL weighted composition + 19.8% RAW savings ⇒ savings spread ~proportionally incl. OUTPUT ⇒ they clear 20% via **OUTPUT reduction (shorter trajectories) + cache stability**, NOT cached-byte volume.
- **SETTLED:** our byte-compressor's WEIGHTED savings ≈ 0 (cached is 0.1-weighted + we bust cache + we don't cut output). Uploading deep would likely FAIL the screener. This matches the ORIGINAL re-derivation §5–6 (output is the lever). The byte-sweep (LATEST-2: 22–31% byte reduction) measured the WRONG proxy — byte VOLUME ≠ weighted savings.
- **REAL LEVERS (hard, unproven for us):** (1) CACHE-STABILITY — stop busting cache (likely the recency-guard rolling transition; fixing it could recover ~12% weighted from cache compression); (2) OUTPUT/TRAJECTORY reduction — the qualifier's real edge; emergent, NOT directly controllable by a message compressor. Local CANNOT measure these at scale (variance ≫ signal) → platform is the only arbiter.
- **⚠️⚠️ CRITICAL (2026-07-08): env vars do NOT reach the compression container.** docker-compose.yml `compression-service.environment` = ONLY `MINER_MODULE_PATH` + `COMPRESSION_MUTATE_REQUEST` (no SOMA_SKEL_* passthrough, no env_file). So EVERY live run used the miner's CODE DEFAULTS (profile=target, recency=ON) regardless of the host `SOMA_SKEL_*` I set. ⇒ the "deep sanity" runs were actually **target** (deep-vs-target step diffs = pure variance); and **recency was ON in ALL runs → we have NEVER compressed the fresh input.** To change miner behavior you MUST edit the code default (or mount a variant), not set an env var.
- **FRESH INSIGHT + TEST (the reframe):** weighted = 1·input + 0.1·cached + 3·output. Each tool result is INPUT (×1.0) the turn it arrives, then CACHED (×0.1) after. Our recency guard kept the fresh result verbatim → we compressed each result ONLY while cheap (cached ×0.1), NEVER while expensive (fresh input ×1.0) = backwards → weighted ≈ 0. **FLIP: compress the fresh input too (recency OFF).** Attacks input ×1.0 directly (locally measurable) + more cache-stable (deterministic, no recency transition) + may shrink per-turn output (less to analyze). Variant built: `miner/cot_compression/upload_miner_skel_recoff_v1.py` (skeleton_v2 + `_RECENCY=False`). Testing live: input-fraction + weighted + patches vs baseline. Signal: recency-off input fraction should be ≪ target's (toward qualifier's 3.0%).
- **(a) DONE (was mislabeled "deep"; actually target+recency-ON):** 100% patch, no spiral — but weighted savings ≈ 0 (expected, recency-on).
- **REDESIGN ATTEMPT (fidelity_v1, USER chose output/trajectory lever):** built `miner/cot_compression/upload_miner_fidelity_v1.py` — cache-stable (position-independent, NO recency exemption), noise-removal + precise re-read markers. Verified: compliance PASS, deterministic, **more cache-stable (35.7% vs skeleton 27.7%)**. BUT fidelity-first ⇒ **only 0.6% byte reduction** ⇒ context ~unchanged ⇒ no behavior/turn change ⇒ ~0 savings. **THE DILEMMA (both extremes fail the weighted gate):** aggressive busts cache + forces re-reads (~0 weighted); gentle changes nothing (~0 weighted). The qualifier's SWEET SPOT (cut ~20% AND fewer turns) needs compression that improves agent efficiency — NOT identifiable locally (turn effect buried under 2–4× step variance). Local analysis EXHAUSTED.
- **HONEST STANDING:** comp-110 is likely NOT winnable via our `compress_messages` approach under the CURRENT weighted formula (structurally disfavors compression: cached ×0.1, output ×3 uncontrollable). Team said the formula is CHANGING to reward input-reduction better → **HOLD is likely higher-EV than platform-experimentation** (low-probability, USER-run uploads). Decision with USER. Candidates unchanged, NOT uploaded. dedup headroom low (msg 0.9% / line 7.0%).

## ★★★ 2026-07-07 (LATEST-2) — [SUPERSEDED by LATEST-3] "20% achievable, we're competitive at 22% byte" — the 22% was byte VOLUME, not weighted; weighted savings ≈ 0. The qualifier-exists finding + method-error corrections still hold.
## ★★★ 2026-07-07 (LATEST-2-orig) — ⚠️ VERDICT CORRECTED: 20% IS achievable (dashboard has a qualifier). My "impossible" call was WRONG — measurement + method errors. Gap is aggressiveness, not a wall.
- **USER caught it:** comp-110 board (snap 2026-07-07/225552) shows **1 of 46 real subs QUALIFIED = `5DtYjEJbjzi6…`** (screener_passed=True; total 1.13). 45 not-qualified, 1 "no api key". So the 20% screener IS clearable.
- **HOW the qualifier did it (detail scrape 225745):** 15 swebench_verified screener runs, 5 django tasks. **Aggregate RAW-token savings = 19.8%** (base 39.9M / miner 32.0M; tokens are 95% cached). Per-task swing −20.7%…+43.7% but the AGGREGATE clears. **Passes correctness** (pass_with=True). Output LOW+stable (~6.4k/run vs our 12k, baseline 12–20k) ⇒ short non-flailing trajectories ⇒ their weighted≈raw.
- **MY ERRORS (corrected):** (1) I measured WEIGHTED savings vs a fresh PAIRED baseline (2–4× step variance); the platform uses a FIXED seeded baseline (`baseline_run=True`, swebench_orchestrator.py `_evaluate_screening_for_script`) aggregated over 15 runs — my flails (66-step outlier) dragged raw-11% → weighted-3.5% in a way the platform method would not. (2) The comparable metric vs the dashboard is RAW: **our 11.0% vs qualifier 19.8%** — we compress ~half as much. (3) Concluding "impossible" from OUR candidate underperforming, ignoring the existing qualifier.
- **CORRECTED READ #2 (BYTE-SWEEP VALIDATED — offline replay reproduces the live sidecar EXACTLY, 0.867 both):** our compression is NOT behind. Target reduces raw context bytes **13% (django-14017 run1) → 22% (448-request django aggregate)**; deep **25%** / ultra **27%** / extreme **31%** (scratchpad/byte_sweep.py over real captured messages-in). Competitive with the qualifier's 19.8% raw; deeper = HEADROOM above 20%. The earlier "11% raw / 3.5% weighted" was TRAJECTORY-VARIANCE-confounded (paired baseline + a 66-step flail), NOT the compression rate. Also: the harness re-sends FULL RAW context every turn (zero [[CMP]] in any messages-in) → compressor works on raw each turn (offline sweep is faithful).
- **REAL BINDING UNCERTAINTIES (not the compression lever):** (1) TRAJECTORY STABILITY — flails inflate turns→tokens and erase the savings (our runs vary 16–66 steps); the qualifier keeps output low/stable. (2) CORRECTNESS (≥50% pass) under aggression — NOT gradeable locally (no swebench eval images). Both are high-variance and only truly settled on the PLATFORM (fixed baseline + 15-run aggregate + real grading).
- **NEXT:** compression is adequate → lean toward `deep` (headroom). Local measurement has given its signal; the residual risk is platform-only. Plan: quick live `deep` sanity (patches produced? no spiral?) → Codex re-audit → USER uploads to a SPARE hotkey for the true screener verdict. Candidate 429d1b26 unchanged, NOT uploaded.

## ★★★ 2026-07-07 (LATEST) — [SUPERSEDED by LATEST-2 above] LIVE eval → 3.5% weighted (paired baseline). Real finding stands (pipeline validated, miner fires) but the "CANNOT clear 20%" verdict was WRONG (see above).
- **USER DIRECTIVE (supersedes the HOLD-for-formula-change posture):** timeline for the formula change is unknown → work under the CURRENT formula; make the candidate perfect + pass all testings NOW.
- **PIPELINE VALIDATED (first time ever):** the comp-110 copilot stack runs e2e locally; miner **FIRES** (36 `compress_messages` calls); solve `completed` + real patch; miner+baseline both 100% patch rate over 6 runs. Offline gates all PASS (harness ×3, rules/compliance). Runners: scratchpad/{run_swebench_eval.sh, agg_swebench_eval.sh}; guide reports/local_testing_guide.md.
- **SCREENER MEASURED** (skeleton_v2 `target`, n=6 pairs = 2 django × 3 runs, ratio-of-sums, cached 1/10): **AGGREGATE WEIGHTED SAVINGS = 3.5% (gate = 20%).** Byte compression is real (~13–20% at equal steps; sidecar out/in 0.867) but SWAMPED by `agent_steps` variance (same miner 16→66 steps across runs); compression does NOT systematically cut steps (miner mean 26.7 vs baseline 27.7). ⇒ the trajectory/output lever (the ONLY math path to 20%) is NOISE, not a reliable positive.
- **VERDICT — CODEX-CONFIRMED (both gates), upstream-verified:** a compliant pure `compress_messages` context compressor **CANNOT clear the 20% screener** under the current formula; profile tuning will not cross it. Codex gate #2 independently re-derived **3.51%** (matches); gate #3 red-team found **no reliable path to 20%** (reaching it needs ~107k more weighted ≈ 67% of output tokens = a behavioral/step-reduction win, not compression). Gate 0.2 / cache 0.1 / input 1.0 / output 3.0 / pass 0.5 VERIFIED from `git show upstream/main:mcp_platform/app/core/config.py` @ 9c1be25 (Codex had read a stale local copy). Refined claim: *byte compression alone cannot clear it; only a proven step-reducing compressor might.* Full data + options: **reports/comp110_local_eval_findings.md**. Codex trail: scratchpad task b3vkp57oo.
- **LIMITS:** correctness (≥50% pass gate) NOT gradeable locally (no per-repo swebench eval images; `--swerebench-eval` = slow/x86); platform is the correctness arbiter. Candidate UNCHANGED (sha 429d1b26), NOT uploaded, NOT committed this session.
...
## ★★★ 2026-07-07 ~05:00Z — ⚠️⚠️ INTERFACE BREAK: comp-108 miners are INCOMPATIBLE with comp-110. Full brief: `reports/comp110_miner_brief.md`. Miner env SET UP.
- **The new agent = the official `@github/copilot` CLI** (Docker Compose sandbox: copilot + proxy + compression-service). Platform executor rewritten upstream (`compact_bench_executor.py`): compression image is now **`soma-copilot-compression-service:latest` from SOMA-benchmark** — the OLD assemble-protocol service is GONE.
- **NEW MINER CONTRACT:** the service **imports the miner as a module** and calls **`compress_messages(messages, path, metadata) -> list`** per outgoing LLM request (OpenAI-style messages array; returned list replaces `payload["messages"]`; module loaded once per run → cross-request state OK; alt names compress_payload/process_request/transform_payload). **No `assemble`, no trajectory JSON.** ⇒ **cap32+pin (695b4fe4) CANNOT be re-submitted as-is — a PORT to `compress_messages` is required.** `_STRUCT_PATTERN`, 32k-cap logic, `[[CMP]]`/`[[BLOCK X]]` markers, loop reasons all carry conceptually; message-array traversal + pairing must be re-engineered. Port = new candidate (build on explicit USER instruction; upload USER-run).
- Model/provider confirmed in code: `SWEBENCH_DEFAULT_MODEL=deepseek/deepseek-v4-pro`, `GATEWAY_FORCE_PROVIDER=deepseek`. 3 benchmark types in `_BENCHMARK_TYPES`: swebench_verified / swe_explorer_explore (regions→`/workspace/explore-result.json`, no patch) / swe_explorer_edit (ground-truth files as hint→patch).
- **Miner env READY on this MacBook:** SOMA-benchmark + SOMA-plugin cloned (`~/joshua-work/`); **SOMA-shared is now PUBLIC** (same 14:36Z release); venv `~/.venvs/soma` = bittensor 9.12.2 + httpx + dotenv; uv present; **both Docker images BUILT** (`local/copilot-cli` 1.46GB, `soma-copilot-compression-service` 707MB). USER-gated (2 cmds): `uv sync` in SOMA-benchmark + pip-install SOMA-shared into the venv. New stack logs `[compression-service][messages.in/out]` per request → local runs can now PROVE the miner fires (comp-108's local eval never could).

## ★★★ 2026-07-07 (latest) — SKELETON v2 = the CONSOLIDATED candidate, CODEX GO. Re-tune harness ready. HOLDING for the (in-flux) score-formula change; NOT uploaded/committed.
- **`upload_miner_skeleton_v2.py` (sha 429d1b26, CODEX GO)** — consolidated: budgeted structural skeleton + file-PATH priority (TRUE budget cap) + LEAN v3 safety + byte-verbatim line endings + PR#176-proofed (documented one-line switch) + all formula-dependent numbers as KNOBS (profiles/env). Direction = team-confirmed intent (reduce input tokens + preserve quality). Verified: harness ALL PASS ×3, rules gate PASS, sidecar E2E, 100% path preservation. Codex: round1 NO-GO (budget-cap + comment tags) → fixed → round2 GO. Frozen: experiments/candidates/SKELETON_comp110_v1/frozen/skeleton_v2_GO_429d1b26.py. Report: BUILD_REPORT_skeleton_v2.md.
- **Re-tune harness** (experiments/candidates/SKELETON_comp110_v1/retune_harness.py): parameterized on (weights, gate); instant tuning when the formula changes. Current constants → nothing clears 20% (by design, holding); hypothetical cached-1/3/gate-10% → target 15.7% clears (proves the tune-and-ship path).
- **⚠️ Under the CURRENT formula (cached 1/10, gate 20%) NO pure context compressor qualifies** (~11% ceiling). Team said the formula is CHANGING to reward input-reduction better → we HOLD building/tuning; watcher (extended to scoring files) fires on the change → then re-derive → retune_harness → smoke → upload decision. Full re-derivation + file:line: reports/comp110_scoring_rederivation.md.
- Lineage frozen as references: LEAN v3 (c8f540b2), SKELETON v1 (3ea9324f). NEXT_ACTIONS TOP has the resume procedure.

## ★★★ 2026-07-07 — LEAN v3 (source-line omission markers) BUILT + CODEX GO. All 10 gates PASS. Smoke blocked ONLY on user .env. Report: BUILD_REPORT_v3.md
- **`upload_miner_lean_v3.py` (sha c8f540b2, Codex-GO)** = LEAN v2 skeleton + LOCATABLE omission markers: elisions emit `[[CMP]] source line N ~ source line M Omitted [[/CMP]]` (the comp-108 king's now-LEGAL Hard edge; README §1 merged 2026-07-07). Kept lines verbatim; NO pin, NO dedupe; final-msg recency default ON (toggle SOMA_LEAN_RECENCY, A/B deferred).
- **Codex gate #1: GO** (round1 NO-GO: stale checked-in README + pin knob → fixed by syncing miner/README_prompting.md from upstream + removing pin; round2 GO). Trail: reports/codex_lean_v3_audit_round1.md + reports/codex_lean_v3_reaudit_GO.md.
- All 10 required gates PASS (rules/Codex/harness/sidecar-E2E/protected-lines/range-accuracy/no-partial/no-net-increase/interior-cache-stable/champion-regression). Frozen: experiments/candidates/LEAN_comp110_v1/frozen/lean_v3_GO_c8f540b2.py.
- ⚠ Coordinate system: "source line N" is BLOCK-relative (template is fixed, can't qualify) → #1 smoke-validation item. Budgets are targets (net-reduction is the guarantee).
- **v2 preserved** (committed 3b8e90a + frozen lean_v2_GO_163be033.py). Both v2 and v3 remain GO; v3 is the smoke candidate.
- **BLOCKED on USER:** `cp .../copilot-cli-container/.env.example .env` (still absent as of ~07:50Z). Then Claude runs the first smoke on v3 TARGET profile (modprobe shim + isolation ON). NO profile A/B until user reviews. NOT committed (v3), NOT uploaded.

## ★★★ 2026-07-07 — LEAN v2 comp-110 candidate BUILT + CODEX GO (gate #1 passed after 2 NO-GO rounds). NOT uploaded. Report: experiments/candidates/LEAN_comp110_v1/BUILD_REPORT.md
- **`miner/cot_compression/upload_miner_lean_v1.py` (LEAN v2, sha256 163be033…)** = the new `compress_messages(messages, path, metadata)->list` candidate. Endurance-first extractive compressor per the postmortem: tiered char-budget cap, whole-line extraction, quality floor (paths/errors/tests/diffs/imports/signatures kept), `[[CMP]]` only on net reduction, compresses ONLY tool+assistant-CoT (never system/developer/user), final-message recency guard, dedupe/pin removed. 3 profiles (SOMA_LEAN_PROFILE) + toggles (SOMA_LEAN_RECENCY, SOMA_LEAN_PIN).
- **VERIFIED:** 30-check harness ALL PASS ×4 configs; real sidecar E2E (compressor loads, 190k→18k, huge user instr verbatim, giant JSON verbatim, interior cache-stable); rules gate GATE+FILE PASS.
- **Codex gate #1: GO** (round1 NO-GO 5 fixes → round2 NO-GO recency/cache → round3 GO). Audit trail: reports/codex_lean_v1_audit.md + reports/codex_lean_v1_reaudit_round2_GO.md. Reconciled disagreement: recency-vs-cache is a fundamental break-safety tradeoff, not a defect (Codex agreed).
- **NEXT (USER):** real smoke solve on the copilot stack (needs OPENROUTER key + `uv run`) to (a) prove firing via [compression-service] logs, (b) get run-level token ratio vs a no-script control, (c) A/B the 3 profiles → pick ~2.5x. Then explore-mode smoke + optional pin/recency A/B → upload decision. NOT committed (awaiting user).
