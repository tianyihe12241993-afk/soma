"""SOMA comp-110 (CoT-Compression-5) — INTERIOR-ULTRA v1.
...
_PROFILES = {
    "ultra":  (300, 1, 1, 250),   # DEFAULT (env does not reach the platform container → this runs)
    "deep":   (400, 2, 1, 380),
    "target": (500, 3, 1, 550),
    "safe":   (700, 5, 2, 900),
}
_PROFILE = os.getenv("SOMA_SKEL_PROFILE", "ultra").strip().lower()
...
def _protected_indices(messages: list[Any]) -> set[int]:
    """The FRESH tail is never compressed: the final message always; and when the request ends in
    tool results, the entire trailing run of consecutive `tool` messages (parallel tool calls)."""
