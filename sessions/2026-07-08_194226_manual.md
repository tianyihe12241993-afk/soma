# Session checkpoint — 2026-07-08T19:42:26Z
**reason:** manual
**NOTE:** capladder-v2 BUILT (upload_miner_capladder2_v1.py): fixes f1km salience defect - top-level-structure-only salience (drop indented bodies/comments/docstrings as CONTIGUOUS runs min6), floor 2000, grep/search + fresh read + system/user/assistant untouched. Tier-0 ALL PASS, compliance PASS, deterministic, markers tile exactly. Offline: cuts VIEW file-reads 20-51pct across the 5 screener tasks (50.8 on 15103) = qualifier zone on the dominant mass; grep results 0pct (self-salient, correct). Emulator pass RUNNING (byo8f542p, 5 lanes x2). Grading deferred to quiet machine + re-grade rule.
