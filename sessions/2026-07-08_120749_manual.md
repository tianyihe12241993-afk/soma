# Session checkpoint — 2026-07-08T12:07:49Z
**reason:** manual
**NOTE:** FROM-SCRATCH PIVOT (user mandate; STOP overturned by qualifier existence proof): contract re-read - message list SHAPE is free (drop/stub whole messages legal; whole-payload replacement exists but unusable - fields not received). NEW ARCH: EVIDENCE-WINDOW v1 (upload_miner_evwindow_v1.py) = stub interior tool results to ONE marker line (no menus, no fragments), fresh tool-run + ALL assistant narrative + system/user verbatim. Matches qualifier footprint (input~baseline, cached~half). Compliance PASS, Tier-0 PASS, offline 29.9pct byte -> ~22.4 weighted PREDICTED ABOVE GATE. Emulator 5x2 running (biknds0kh). Open question = correctness/steps (where deep380 died; design differs on marker-menu + narrative-intact).
