# Session checkpoint — 2026-07-08T13:15:59Z
**reason:** manual
**NOTE:** COPILOT AGENT PROFILE complete (reports/copilot_agent_profile.md): no client compaction (shrinks=sub-agent threads); flail rate baseline 12pct vs candidates 16pct - flails are mostly agent lottery (wrong-first-fix test loops, bash+18/tests+9), compression adds ~4pts; markers-steps corr +0.65 largely reverse-causation; evw refuted by agent behavior (re-acquires deleted evidence); Gate B = f(steps) = stochastic; screener substantially a draw lottery around sub-20 mean. Recommendation: HOLD + comp-111 prep; lottery shot = USER decision only.
