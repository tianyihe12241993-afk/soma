...
NP_MIN_FLOOR = 16_000          # results <= this PASS THROUGH = IDENTICAL to np2's 16k cap -> preserves Easy 1.052 + Medium 0.849.
                               # *** NOCAP VARIANT = behavioral reconstruction of the UPLOADED version (sha 96052567). NO upper cap
                               # on the keep: a HUGE result keeps 0.60*len UNCAPPED (100k -> 60k). This is the version that is LIVE on
                               # the hotkey. Its sibling upload_miner_uphard_cap32.py caps huge results at 32k (np3's proven Hard zone)
                               # to avoid np5's near-passthrough crater (48k -> 0.25) on the biggest contexts. Behavioral note: this
                               # file's bytes differ from the live 96052567 only in COMMENTS (logic identical) -> for audit/reference. ***
NP_RESULT_CAP = NP_MIN_FLOOR   # alias kept for the metadata field below; the EFFECTIVE cap is proportional (see cap_tool_result).
...
def cap_tool_result(message, cap):
    ...
    cap = max(NP_MIN_FLOOR, int(len(text) * NP_KEEP_FRAC))  # NOCAP: no upper bound on the keep (uploaded/live version)
    if len(text) <= cap:
        return message, False
