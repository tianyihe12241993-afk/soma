...
# v23 GENTLER-HARVEST for easy/low-work tasks. Platform read: our Easy (1.127) is
# dragged by over-aggressive harvest causing wander/breaks on a few easy tasks,
# while 2nd-place (same harvest recipe family) hits Easy 1.433. The compressor
# can't tell Easy from Medium at runtime, so we bias gentleness by TOTAL WORK:
# low-cumulative (easy-leaning) tasks harvest to a gentler target (keep more ->
# fewer over-compression breaks); high-cumulative (medium-leaning, where the comp
# bonus actually matters) keep the aggressive 8k. Medium exposure is limited to
# small tasks where 8k-vs-gentler is a small absolute difference anyway.
EASY_HARVEST_TARGET = 14_000   # gentler harvest target for low-cumulative tasks
EASY_CUM_GATE = 300_000        # cumulative-observed below this -> gentler harvest
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "passthrough":
        ...
        sanitized = working
        # v23: gentler target for low-work (easy-leaning) tasks; aggressive 8k for
        # high-cumulative (medium-leaning) where the comp bonus matters.
        harvest_target = (
            EASY_HARVEST_TARGET if cumulative_observed < EASY_CUM_GATE else TARGET_TOKENS
        )
        result_messages, info = compress_structurally(
            working, escalation, target_tokens=harvest_target
        )
