...
def detect_loop(messages: list[Any]) -> str:
    """v19: same tool call producing a NEAR-identical result LOOP_THRESHOLD+ times
    in the recent window — repetition that yields no new information. v18 hashed
    the byte-identical result, which missed the common wander pattern (re-running
    a command that keeps failing with only incidental differences — line numbers,
    counts, timestamps). v19 hashes the near-dup normal form (digits/punct dropped,
    the same normalization compression uses for near-duplicate collapse), so
    "still failing the ~same way" counts as a loop. Detection only — the coach
    message stays a neutral loop flag (no workflow steering), per the 2026-06-13
    organizer ruling that limits notes to loop-detection + forced-stopping."""
    result_hash: dict[str, str] = {}
    for message in messages:
        ids = extract_tool_result_ids(message)
        if ids:
            digest = hashlib.sha256(
                _norm_for_neardup(extract_text(message.get("content"))).encode("utf-8")
            ).hexdigest()
            for rid in ids:
