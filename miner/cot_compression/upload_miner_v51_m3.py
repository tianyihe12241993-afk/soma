...
CHARS_PER_TOKEN = 4
# v3 three-mode budget. Platform data (CoT-Compression-3) showed the elite
# don't out-compress us — they out-stabilize us: the savings gate is saturated
# for everyone, and run-level flakiness on medium-length tasks is what costs
# points. So: leave short contexts untouched (cache-warm), only *truncate*
# mid-size contexts (never drop, protect error output), and keep the tight
# v2 machinery for huge contexts where timeout rescue wins +4s.
PASS_THROUGH_TOKENS = 12_000   # below: return unchanged (changed=False)
TIGHT_TOKENS = 40_000          # observed-size ratchet; tight mode is sticky
TARGET_TOKENS = 8_000         # tight-mode trajectory target
PRESSURE_RELIEF = 0.75

# Gentle mode (12k-40k observed): truncation-only caps, in chars.
GENTLE_RESULT_CAP = 4_000
GENTLE_PROTECTED_CAP = 12_000
...
# Tail protection is split by mode: gentle buys fidelity (its whole point),
# tight buys speed — bigger protected tails there cost tokens per call exactly
# where timeout rescue wins +4s (v3.0's 14539 regression).
GENTLE_KEEP_RECENT = 6
...
# Tight-mode coach: a static discipline reminder appended as the final message
# on long (hard) tasks, where baselines usually fail and the observed agent
# failure modes are procedural (edited a copy of the file, left debug scripts,
# finished without changes, skipped running tests). Static text + always-last
# placement keeps it cache-cheap: only its own tokens ever re-bill.
COACH_MARKER = "[SOMA TASK DISCIPLINE]"
COACH_TEXT = (
    "[SOMA TASK DISCIPLINE]\n"
    "Automated reminder from your workflow tooling (the original task is in the first message):\n"
    "- Re-read the original issue before concluding; the fix must change the exact reported behavior.\n"
    "- Edit the real source file in the repository — never a copy, a new variant, or a scratch file.\n"
    "- After editing, run the narrowest relevant tests. If one fails, read the full error output before editing again.\n"
    "- Keep the final patch minimal: no debug scripts, no summary documents, no unrelated changes.\n"
    "- Delete any scratch or debug files you created before finishing."
)
...
def build_coach_text(messages: list[Any], *, governor: bool) -> str:
    ...
    if issue:
        parts.append("The original task, restated verbatim for reference:")
        parts.append(issue)
    status = extract_test_status(messages)
    if status:
        parts.append(
            "Current failing-test status (from the latest test output in this session):"
        )
        parts.append(status)
        parts.append("Verify your fix against these exact tests before concluding.")
    loop = detect_loop(messages)
    if loop:
        parts.append(
            f"NOTE: you have repeated the same action with an identical result "
            f"multiple times: {loop}. Repeating it again will not produce new "
            f"information — re-read the task above and try a different approach."
        )
    if governor:
        parts.append(
            "You have already used substantial effort on this task. If the relevant "
            "tests pass with your fix, finish NOW with the final answer; do not "
            "explore further."
        )
    return "\n".join(parts)
...
def compress_gently(messages: list[Any]) -> tuple[list[Any], dict[str, Any]]:
    ...
    tail_results = set(result_indices[-GENTLE_KEEP_RECENT:])
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult" and index not in untouchable:
            if index in duplicate_indices:
                ...
                cap = GENTLE_PROTECTED_CAP if protected else GENTLE_RESULT_CAP
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if prev_mode == "tight" or observed >= TIGHT_TOKENS:
        mode = "tight"
    elif prev_mode == "gentle" or observed >= PASS_THROUGH_TOKENS:
        mode = "gentle"
    else:
        mode = "passthrough"
    ...
    if mode == "passthrough":
        ...
        if mode == "gentle":
            # No sanitization in gentle mode: stay byte-identical to the
            # native context everywhere except the truncated old results, so
            # the provider's cached prefix survives the mode transition.
            sanitized = working
