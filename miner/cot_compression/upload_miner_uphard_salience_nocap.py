#!/usr/bin/env python3
"""SOMA miner uphard_salience_NOCAP (UNCAPPED keep + salience import-pin, cache-stable) — derived from salience/m12.
...
NP_MAX_KEEP = 1_000_000_000    # ⚠️ NOCAP VARIANT: sentinel = effectively UNCAPPED (min() never binds) = nocap's keep
                               # max(16k, 0.60*len) + the salience import-pin. Reintroduces nocap's uncapped-tail crater RISK
                               # (live nocap 5FXAR4 = 0.416, M0.266, 17% breaks) but adds _STRUCT_PATTERN to convert nocap's
                               # import-heavy breaks (django/sympy) -> passes. Differs from salience(cap32) & cap64 ONLY in this
                               # constant. Highest Overall ceiling, highest risk. Net map: <=16k pass(=np2) | >16k keep 0.60 (uncapped).
...
# SALIENCE FIX (2026-06-29, reports/reanalysis_2026-06-29.md): the base extractive PINS def/class/errors/
