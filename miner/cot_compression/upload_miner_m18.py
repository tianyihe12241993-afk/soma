#!/usr/bin/env python3
"""SOMA miner m18 — m12 (m7_compliant) + size-gated ADAPTIVE-LIGHT compression.

m18 SINGLE-VARIABLE CHANGE vs m12: a new "ultralight" mode that DOWNGRADES the
harvest path to near-passthrough for clearly small / easy-like (SHALLOW) contexts,
while leaving everything else byte-for-byte m12. Motivation (5DAh2rUM data): m12's
aggressive harvest over-compresses easy tasks and breaks ~18% of their runs (Easy
0.412); near-passthrough on those preserves the agent's native solving (the king
got Easy 0.853 / ~6% breaks at ~1.0× ratio). But uniform lightness is FATAL on Hard
(the king collapsed to H=-3.6), so m18 keeps m12's aggressive harvest/rich on
large/hard contexts — the Hard moat (m12 H=0.919) is protected by construction.

WHAT ENTERS ULTRALIGHT (compress_ultralight): only when the m12 router would pick
HARVEST, the task has never escalated (prev_mode in passthrough/ultralight), the
error guard is quiet, there is no prior harvest pressure, AND depth/tokens/cumulative
are all clearly small (ULTRALIGHT_MAX_MSGS/TOKENS/CUM). Any uncertainty -> m12 harvest.
Monotonic: passthrough -> ultralight -> harvest -> rich, never reverts.

ULTRALIGHT behavior: identical dedup machinery to compress_gently (exact + near-dup
old tool results -> allowed "Same response as in [[BLOCK N]]." back-reference) but
NON-duplicate results are kept BYTE-INTACT except a generous BLIND head/tail cap
(ULTRALIGHT_RESULT_CAP) on pathologically huge single results. NO extractive/TF-IDF
selection or reordering (that is the m17 perturbation we are avoiding) — lowest
perturbation, original order + recent window preserved. Savings come from dedup.

LARGE/HARD contexts: rich + non-shallow harvest are UNCHANGED from m12 (byte-identical).
COMPLIANT: deterministic only; no LLM/API call; no task-id/benchmark/category logic
(gates on raw structural size signals only); no behavior steering; only the allowed
CMP/BLOCK markers + loop-reason strings. GATED EXPERIMENT — NOT submitted; m12 stays LIVE.
...
# ----- m18 ULTRA-LIGHT gate (the SINGLE change vs m12) ----------------------
# A size-gated, near-passthrough DOWNGRADE of the HARVEST path for clearly
# small / easy-like (shallow) contexts only. NEVER touches rich (the Hard moat)
# or harvest on non-shallow contexts. Thesis (from the 5DAh2rUM data): m12
# over-compresses easy tasks and breaks ~18% of their runs; near-passthrough on
# those preserves the agent's solving (king got Easy 0.85 / 6% breaks at ~1.0x).
# Aggressive harvest stays on large/hard contexts (king died there at H=-3.6).
# CONSERVATIVE by design — if any signal is uncertain, fall back to m12 harvest.
# These are the A/B-TUNABLE knobs; depth is the primary signal (per the routing
# comment below, per-call tokens don't separate easy from hard — depth does).
ULTRALIGHT_MAX_MSGS = 40       # clearly shallow (m12 trips rich at 90; error-guard starts at 40)
ULTRALIGHT_MAX_TOKENS = 60_000 # clearly small per-call (m12 trips rich at 120k) — secondary net
ULTRALIGHT_MAX_CUM = 250_000   # little processed so far (m12 trips rich at 600k cum) — secondary net
ULTRALIGHT_RESULT_CAP = 40_000 # chars (~10k tok): ONLY pathologically huge single results get a blind head/tail cap; everything else is preserved intact
...
def compress_ultralight(messages: list[Any]) -> tuple[list[Any], dict[str, Any]]:
    # m18 ULTRA-LIGHT (small/easy-like contexts only). The LOWEST-perturbation
    # compression we ship: identical dedup machinery to compress_gently (exact +
    # near-duplicate old tool results -> the allowed "Same response as in
    # [[BLOCK N]]." back-reference), but NON-duplicate results are kept BYTE-INTACT
    # except a generous BLIND head/tail cap (ULTRALIGHT_RESULT_CAP) that fires only
    # on pathologically huge single results. NO extractive/TF-IDF selection or
    # reordering (that is the m17 perturbation we are avoiding), original order and
    # context shape preserved, recent working window untouched. Savings come from
    # dedup; the agent sees ~native context -> fewer pass-pass run breaks.
    info: dict[str, Any] = {
        "truncatedResultCount": 0,
        "duplicateResultCount": 0,
        "nearDuplicateResultCount": 0,
    ...
    # Newest-first dedup registries (identical policy to compress_gently): exact-dup
    # back-ref is safe for any result; near-dup back-ref only for old, non-load-bearing.
    active = frozenset(active_paths(messages))
    ...
    cap_head, cap_tail = int(ULTRALIGHT_RESULT_CAP * 0.7), int(ULTRALIGHT_RESULT_CAP * 0.3)
    output: list[Any] = []
    for index, message in enumerate(messages):
        role = normalize_role(message.get("role")) if isinstance(message, dict) else ""
        if role == "toolResult" and index not in intact:
            if index in dup_ref:
                ...
                # NON-duplicate: keep INTACT; only a pathologically huge single
                # result gets a BLIND head/tail cap (preserves order; no extraction).
                message, did = truncate_message(message, cap_head, cap_tail)
                info["truncatedResultCount"] += int(did)
        elif role == "toolResult" and index in block_number:
            message = _wrap_first_text_field(message, block_open(block_number[index]), block_close(block_number[index]))
        output.append(message)
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # m18: size-gated ULTRA-LIGHT downgrade of the HARVEST path for clearly small /
    # easy-like (shallow) contexts. Conservative + monotonic: only from
    # passthrough/ultralight (NEVER downgrade an already-escalated harvest/rich
    # task), only when depth AND token AND cumulative are all clearly small AND the
    # error guard is quiet AND no prior harvest pressure. ANY uncertainty -> stays
    # m12 harvest. Rich (the Hard moat) and harvest on non-shallow contexts are
    # left byte-identical to m12. Depth is the primary signal (per the routing note
    # above, per-call tokens don't separate easy from hard — depth does).
    if (
        mode == "harvest"
        and prev_mode in ("passthrough", "ultralight")
        and not still_failing
        and not escalation
        and msg_depth <= ULTRALIGHT_MAX_MSGS
        and observed <= ULTRALIGHT_MAX_TOKENS
        and cumulative_observed <= ULTRALIGHT_MAX_CUM
    ):
        mode = "ultralight"
    ...
    if mode == "passthrough":
        ...
        # m18: near-passthrough + safe dedup for small/easy-like shallow contexts.
        # Lowest-perturbation path (no extractive selection); preserves order, recent
        # window, and tracebacks/test/diff/code lines intact. Same orphan guard +
        # empty-output fallback + compliant loop guard as the other modes.
        sanitized = working
        result_messages, info = compress_ultralight(working)
        pruned = info.get("reason") == "ultralight"
