...
def compress_gently(messages: list[Any]) -> tuple[list[Any], dict[str, Any]]:
    ...
    active = frozenset(active_paths(messages))  # files/tests the agent is currently working
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult" and index not in untouchable:
            if index in duplicate_indices:
                ...
                # v8: rule-first extractive summarization at the v6 char budget,
                ...
                message, did = extractive_message(message, cap, active)
