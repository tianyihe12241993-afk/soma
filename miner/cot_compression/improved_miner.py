...
STEADY_TOKENS = 12_000         # harvest below, gentler "steady" band above
...
# v6.1: size proxies difficulty. Small contexts (≈easy tasks) get AGGRESSIVE
# harvest — short runs tolerate hard truncation and the token bonus is pure
# upside. Mid contexts (≈medium tasks) get the GENTLER "steady" band: medium is
# where aggressive truncation risks the agent losing the thread and wandering
# into a longer (more expensive) run — m2's gentler mode leads the board's
# Medium at 1.119, so we keep mid-size context fuller. Large (≈hard) → tight.
HARVEST_RESULT_CAP = 1_200     # aggressive: old non-error results trimmed hard
HARVEST_PROTECTED_CAP = 5_000
HARVEST_KEEP_RECENT = 4
STEADY_RESULT_CAP = 3_500      # gentle: keep mid-size context fuller (m2-like)
STEADY_PROTECTED_CAP = 10_000
STEADY_KEEP_RECENT = 6
# Back-compat aliases (tight-mode planner still references these names).
GENTLE_RESULT_CAP = HARVEST_RESULT_CAP
GENTLE_PROTECTED_CAP = HARVEST_PROTECTED_CAP
...
# Untouchable newest results protect the agent's active state in every mode.
...
COACH_HEADER = (
    "[SOMA CONTEXT NOTE]\n"
    "Automated context-management note from your workflow tooling (not from the user)."
)
...
def build_coach_text(messages: list[Any], *, governor: bool) -> str:
    # Composed only from the three permitted classes; each block is labeled
    # with which one it is so compliance is auditable from the output itself.
    parts = [COACH_HEADER]

    # (a) Context-management note: a summary of the original task.
    issue = extract_issue_text(messages)
    if issue:
        parts.append("Summary of the original task, for reference:")
        parts.append(issue)

    # (c) Forced stopping with a precise completion definition. Defining the
    # stopping state as "tests pass via a minimal change to existing source"
    # IS the stopping condition (an allowed technique), and orienting the agent
    # toward that end-state curbs the scratch-file wandering the cut directives
    # used to suppress — without any generic behavioral directive.
    status = extract_test_status(messages)
    if status:
        parts.append("Stopping condition — the task is complete when these tests pass:")
        parts.append(status)
    parts.append(
        "The task is complete once these tests pass through a minimal change to the "
        "existing repository source file(s); when you reach that state, stop and "
        "return your final answer rather than continuing to explore."
    )
    ...
    # (c) Forced stopping, escalated once substantial effort has been spent.
    if governor:
        parts.append(
            "You have spent substantial effort already. If the stopping condition "
            "above is met, STOP NOW and return the final answer; do not explore further."
        )
    return "\n".join(parts)
...
def compress_gently(
    messages: list[Any],
    *,
    result_cap: int = HARVEST_RESULT_CAP,
    protected_cap: int = HARVEST_PROTECTED_CAP,
    keep_recent: int = HARVEST_KEEP_RECENT,
    ...
    untouchable = set(range(max(0, len(messages) - GENTLE_UNTOUCHABLE), len(messages)))

    # Newest-first hash registry so older exact duplicates collapse.
    ...
    for index in reversed(result_indices):
        ...
        if digest in seen_hashes and index not in tail_results and index not in untouchable:
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult" and index not in untouchable:
            ...
            elif index in tail_results:
                message, _ = truncate_message(
                    message, int(ABS_RESULT_CAP * 0.75), int(ABS_RESULT_CAP * 0.25)
                )
            else:
                protected = is_error_bearing(message)
                cap = protected_cap if protected else result_cap
                message, did = truncate_message(message, int(cap * 0.75), int(cap * 0.25))
                info["truncatedResultCount"] += int(did)
                info["protectedResultCount"] += int(protected and not did)
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # ever escalate (passthrough -> harvest -> steady -> tight), never revert.
    observed = max(
        estimated,
    ...
    rank = {"passthrough": 0, "harvest": 1, "steady": 2, "tight": 3}
    if observed >= TIGHT_TOKENS:
        mode = "tight"
    elif observed >= STEADY_TOKENS:
        mode = "steady"
    elif observed >= PASS_THROUGH_TOKENS:
        mode = "harvest"
    else:
        mode = "passthrough"
    # Monotonic ratchet: never de-escalate below where we've already been.
    if rank.get(prev_mode, 0) > rank[mode]:
        mode = prev_mode
    escalation = dict(extras.get("escalation") or {})

    # Our coach message from previous rounds rides in via state; remove it so
    # compression never sees it, and re-append (tight mode only) at the end.
    ...
    if mode == "passthrough":
        ...
        if mode in ("harvest", "steady"):
            # Truncation-only modes: no sanitization, so the native context
            # stays byte-identical except for truncated old results and the
            # provider's cached prefix survives the mode transition. Harvest
            # (small/easy) trims hard; steady (mid/medium) keeps context fuller.
            sanitized = working
            if mode == "harvest":
                result_messages, info = compress_gently(
                    working,
                    result_cap=HARVEST_RESULT_CAP,
                    protected_cap=HARVEST_PROTECTED_CAP,
                    keep_recent=HARVEST_KEEP_RECENT,
                )
            else:
                result_messages, info = compress_gently(
                    working,
                    result_cap=STEADY_RESULT_CAP,
                    protected_cap=STEADY_PROTECTED_CAP,
                    keep_recent=STEADY_KEEP_RECENT,
                )
            pruned = info.get("reason") == "gentle"
        else:
            sanitized, sanitize_changed = sanitize_messages(working)
            governor = observed >= GOVERNOR_TOKENS
            result_messages, info = compress_structurally(
                sanitized,
                escalation,
                target_tokens=GOVERNOR_TARGET_TOKENS if governor else TARGET_TOKENS,
            )
