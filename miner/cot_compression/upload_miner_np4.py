...
NP_RESULT_CAP = 16_000         # np4: back to np2's 16k cap = our BEST Overall base (np2 #2, total 0.684, E1.052/M0.849).
                               # np4 = np2 cap + a CACHE-STABLE LOSSLESS EXACT-DEDUP pass (see dedup_exact_results). RATIONALE
                               # (deep-dive §15-E): #1 (5DCnA57, 0.697) beats us on Overall PURELY by run-stability (9 hidden
                               # breaks vs our 11-14), NOT compression — our CLEAN-run mean is HIGHER (np3 1.206 > #1 1.157). #1 is
                               # near-pure-passthrough → it leaves duplicate-content token savings on the table. Lossless dedup
                               # (replace a byte-identical REPEATED tool result with the allowed "Same response as in [[BLOCK N]]."
                               # back-ref) harvests those savings → lifts our clean-run ratio FURTHER above #1 → enough to overcome
                               # our break-drag on the Overall mean (clean fraction ~0.7 × +bonus > #1's 0.697). It is LOSSLESS
                               # (content survives verbatim at the survivor) → no info removed → NO new agent-divergence/breaks
                               # (the right lever; we already out-SAVE #1, the gap is breaks). ADDITIVE BET: np2/np3 stay LIVE
                               # holding 23.8%; np4 can only gain Overall, never lose income. Cache-safety GATED by prefix_stability.
NP_DEDUP_MIN_CHARS = 800       # only dedup an EXACT-duplicate result larger than this (the [[BLOCK N]] label + back-ref cost a few
                               # dozen bytes; below this the dedup would not be a net token win). Lossless: exact (collapsed-ws) only.
...
def cap_tool_result(message, cap):
    ...
    if normalize_role(message.get("role")) != "toolResult":
        return message, False
    text = extract_text(message.get("content"))
    if CMP_START in text or len(text) <= cap:
        return message, False
...
_BLOCK_MARK_RE = re.compile(r"\[\[/?BLOCK \d+\]\]")
_SAME_REF_PREFIX = "Same response as in [[BLOCK"


def _strip_block_markers(text: str) -> str:
    return _BLOCK_MARK_RE.sub("", text).strip("\n")


def dedup_exact_results(messages: list[Any]) -> tuple[list[Any], int]:
    """CACHE-STABLE LOSSLESS exact-dedup of repeated tool results.

    The EARLIEST occurrence of a given tool-result content is the SURVIVOR — it is kept VERBATIM, so the
    cacheable emitted prefix is never rewritten. Each LATER byte-identical copy is replaced by the allowed
    back-reference `Same response as in [[BLOCK N]].`, and the survivor is labelled [[BLOCK N]]…[[/BLOCK N]]
    where **N = the survivor's message INDEX** — which is STABLE as the agent appends turns (earlier indices
    never shift), so block numbers NEVER renumber and the prefix stays byte-stable turn-to-turn (unlike the
    m12 rich dedup, which made the NEWEST copy the survivor and renumbered → cache-hostile).

    PURE + DETERMINISTIC. IDEMPOTENT: an existing back-ref is left untouched, and an already-labelled
    survivor is recognised (its INNER content is registered so a later raw copy still references it) and not
    re-labelled — so running np4 on its own output is byte-identical. LOSSLESS: only EXACT (collapsed-ws)
    duplicates collapse, and the content survives verbatim at the survivor → no information removed → no
    agent-trajectory divergence (the whole point: we already out-SAVE #1; this harvests the duplicate-token
    savings #1 leaves on the table WITHOUT adding break risk). Already-[[CMP]] (capped) results are skipped.
    NO-INFLATION: a copy is replaced only if the back-ref is strictly shorter. Returns (messages, n_deduped)."""
    first_idx: dict[str, int] = {}      # inner-content hash -> survivor (earliest) index
    dup_of: dict[int, int] = {}         # later-duplicate index -> survivor index
    already_labelled: set[int] = set()  # survivors that already carry their [[BLOCK i]] label (don't re-wrap)
    for i, message in enumerate(messages):
        if not isinstance(message, dict) or normalize_role(message.get("role")) != "toolResult":
            continue
        text = extract_text(message.get("content"))
        if not text or _SAME_REF_PREFIX in text or CMP_START in text:
            continue                    # back-ref / capped → leave as-is (idempotent)
        is_labelled = block_open(i) in text
        inner = _strip_block_markers(text) if is_labelled else text
        if len(inner) < NP_DEDUP_MIN_CHARS:
            continue
        digest = hashlib.sha256(collapse_ws(inner).encode("utf-8")).hexdigest()
        if digest in first_idx:
            dup_of[i] = first_idx[digest]
        else:
            first_idx[digest] = i
            if is_labelled:
                already_labelled.add(i)
    if not dup_of:
        return messages, 0
    survivors_to_label = set(dup_of.values()) - already_labelled
    output: list[Any] = []
    n_deduped = 0
    for i, message in enumerate(messages):
        if i in dup_of:
            ref = same_response_ref(dup_of[i])          # N = survivor index (stable)
            if len(ref) < len(extract_text(message.get("content"))):
                output.append(_replace_text_fields(message, ref))
                n_deduped += 1
                continue
            output.append(message)
        elif i in survivors_to_label:
            output.append(_wrap_first_text_field(message, block_open(i), block_close(i)))
        else:
            output.append(message)
    return output, n_deduped
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # CACHE-STABLE LOSSLESS EXACT-DEDUP (np4): collapse byte-identical REPEATED tool results to the allowed
    # back-reference, harvesting the duplicate-token savings #1 (pure passthrough) leaves on the table. Runs
    # AFTER the per-message cap (already-[[CMP]] capped results are skipped). Earliest-survivor + stable
    # block-INDEX label keeps the emitted prefix byte-stable turn-to-turn; lossless ⇒ no agent divergence.
    # A no-op (returns the same list) when there are no exact duplicates ≥ NP_DEDUP_MIN_CHARS.
    result_messages, n_deduped = dedup_exact_results(result_messages)

    # Orphan guard (defensive): neither the per-message cap nor the dedup drops a message (dedup only
    # replaces a duplicate's CONTENT with a back-ref), so call/result pairing is preserved; still verify
    # against the sanitized input and fall back to it on any violation.
    ...
    metadata = {
        "changed": changed,
        "reason": "near_passthrough" if (n_capped or n_deduped) else "passthrough_native",
        "sanitized": sanitize_changed,
        "mode": "near_passthrough_dedup",
        "resultsCapped": n_capped,
        "resultsDeduped": n_deduped,
