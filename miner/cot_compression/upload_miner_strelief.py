...
# stability_relief: results carrying BREAK SIGNALS (tracebacks / assertion / test-failure text) get a
# RELIEVED (gentler) cap so the agent's debugging context survives -> fewer pass->fail breaks -> higher
# Hard, WITHOUT globally raising the cap (which craters Medium, per np3/np_prop). Content-only signal
# (no task/category/benchmark awareness) -> compliant; deterministic -> cache-stable. Effectively =
# np2 (16k) for normal results + np3 (28k) for debugging results.
RELIEF_CAP = 28_000
_BREAK_SIGNAL_RX = re.compile(
    r"Traceback \(most recent call last\):"        # real Python traceback (failure OUTPUT, not a mention)
    r"|^={3,}\s*(?:FAILURES|ERRORS)\s*={3,}"        # pytest failures/errors section header
    r"|^E\s{3,}\S"                                   # pytest error-detail line ("E    AssertionError: ...")
    r"|^FAILED\s+\S+::",                             # pytest "FAILED path::test" result line
    re.MULTILINE,
)


def _break_prone(text: str) -> bool:
    """True if the result text carries objective debugging/failure signals (deterministic, content-only)."""
    return bool(_BREAK_SIGNAL_RX.search(text))


def cap_tool_result(message, cap):
    """Cache-stable per-message cap with STABILITY RELIEF. A tool result that carries break signals is
    capped at RELIEF_CAP (gentler, = np3) instead of `cap` (= np2), keeping debugging context fuller to
    reduce pass->fail breaks. PURE + DETERMINISTIC (active=frozenset(); content-only, no recency/global
    state) and IDEMPOTENT (already-[[CMP]]/already-small results returned UNCHANGED) -> emitted prefix is
    byte-identical turn-to-turn -> provider cache stays warm. Non-tool-result messages pass through."""
    if not isinstance(message, dict):
        return message, False
    if normalize_role(message.get("role")) != "toolResult":
        return message, False
    text = extract_text(message.get("content"))
    eff_cap = RELIEF_CAP if _break_prone(text) else cap   # content-gated relief (np3 cap for debugging turns)
    if CMP_START in text or len(text) <= eff_cap:
        return message, False
    inner_cap = max(256, eff_cap - len(CMP_START) - len(CMP_END) - 2)
