...
NP_RESULT_CAP = 8_000          # hardspec: AGGRESSIVE cap (between m26 6k and np2 16k — an untested point). THESIS:
                               # 5GgVXz wins Hard (0.676) at ratio ~1.74-2.14x by compressing HARDER and landing FLIPs
                               # (noise removal lets the agent focus on the failing test/diff). extractive_compress still
                               # PINS error/test/diff/sig lines, so harder capping drops boilerplate but keeps the debugging
                               # essence -> chase Hard FLIPs. RISK: aggression gave m26 only H0.170 (cap-value alone may not
                               # replicate 5GgVXz's mechanism); SEPARATE hotkey, Medium expected to drop. Platform-only test.
