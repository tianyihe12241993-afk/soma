#!/usr/bin/env python3
"""SOMA miner m12.1b_run_stability — run-variance-tuned derivative of m12 (upload_miner_m7_compliant.py).

WHAT'S NEW vs m12 (all ALGORITHMIC — no new prompt text; only the allowed [[CMP]]/[[BLOCK N]] markers and
the two loop-reason strings are ever emitted, so compliance is unchanged):
  (1a) NEVER-INFLATE GUARD (kept from m12.1, the proven pure-win): if the compressed output's token estimate
       is >= the raw input's, discard it and emit PASS-THROUGH (changed=False). Guarantees ratio >= 1.0 — the
       miner can never grow the context. Kills the <1x inflated-task drag.
  (2)  BOUNDED AGGRESSIVENESS CAP (NEW — the key fix for the 10-20x break-run outliers): in the harvest path
       the non-duplicate sacrifice floor is raised so a single call never compresses below
       total_chars / MAX_COMPRESSION_RATIO. Exact-duplicate stale tool spam is still dropped freely (it runs
       before the budget check), but load-bearing content is WIDENED/retained instead of stripped to an
       ultra-small ~8k context. Binds only when the per-call input exceeds ~64k tokens (where m12's 8k target
       produced >8x), so normal sub-8x Medium/Hard compression is UNCHANGED.
  (3)  DETERMINISM: audited — the engine is already byte-deterministic on identical input (extractive keep-set
       is sorted before output and ranked by (-score, index); path/basename sets are membership-only; digests
       are sha256; counts use insertion-ordered dicts; no time/random/uuid in EMITTED text). Proven by an
       offline same-input->byte-identical fixture. No risky rewrites were needed.
  (4)  TIGHTENED GENTLE ROUTING (vs m12.1's Medium leak): KEEP only the persistent break-prone signal —
       repeated/oscillating identical failure across recent turns -> rich. The error guard stays at m12's
       CONSERVATIVE thresholds (40/4/4, NOT m12.1's over-sensitive 24/6/3). m12.1's shallow_small -> gentle
       route is DROPPED (it was the Medium-leak culprit). Normal compressible Medium stays in harvest.
  (5)  PARTIAL-FLIP STABILIZATION is achieved by (2)+(3): deterministic compression + the cap preserve enough
       context on deep/hard (baseline-fail) trajectories that a winnable flip lands on more of the 5 runs,
       instead of being stripped to a 1/5 or 3/5 partial flip. No flip-specific or task-id heuristic is added
       (that would over-fit and risk compliance).
...
# m12.1b (2): bounded aggressiveness cap. The harvest path never sacrifices NON-duplicate content below
# total_chars / MAX_COMPRESSION_RATIO, so a single compression call can't hit the destructive 10-20x ratios
# that broke baseline-winnable runs. Exact-duplicate stale spam is still dropped freely (it runs before the
# budget check). Binds only when per-call input > ~64k tokens; smaller (normal Medium) contexts are unchanged.
MAX_COMPRESSION_RATIO = 8.0
# m12.1b (4): repeated/oscillating-failure guard (the ONE persistent break-prone signal kept from m12.1).
# The SAME normalized failing-test/assertion line recurring across the recent tool results = "stuck on the
# same break" -> route to the gentle rich path. General transcript signal only (no task-id, no SWE hints).
# Gated on depth so a brand-new task re-running one test once or twice is not swept in. m12.1's shallow_small
# route and its over-sensitive 24/6/3 error guard are NOT carried over (they leaked Medium).
REPEAT_FAIL_MIN_MSGS = 24
REPEAT_FAIL_WINDOW = 8
...
def _failure_lines(message: Any) -> set[str]:
    """Normalized failing-test / assertion lines from a tool result (lines matching
    TEST_LINE_PATTERN). Normalized like near-dup detection so the same failure with
    shifting line numbers/counts compares equal — the recurring signature of a break."""
    if not isinstance(message, dict) or normalize_role(message.get("role")) != "toolResult":
        return set()
...
def repeated_failure(messages: list[Any], window: int = REPEAT_FAIL_WINDOW) -> bool:
    """True if the SAME normalized failure line recurs across the recent `window` tool
    results >= REPEAT_FAIL_THRESHOLD times — an objective 'stuck on the same break'
    signal. General transcript signal only (no task-id / SWE hints). Order-independent
    result (set iteration only feeds a threshold count), so it stays deterministic."""
    counts: dict[str, int] = {}
    seen = 0
    for message in reversed(messages):
        if not isinstance(message, dict) or normalize_role(message.get("role")) != "toolResult":
...
def compress_structurally(
    ...
    # m12.1b (2): bounded aggressiveness cap. Floor the budget so non-duplicate
    # sacrifice never drives the output below total_chars / MAX_COMPRESSION_RATIO
    # (the per-call ratio can't exceed MAX_COMPRESSION_RATIO via content loss).
    # Exact-duplicate drops still run before the budget check, so stale tool spam is
    # removed freely. This widens retained context on big harvests (the 10-20x break
    # runs) while leaving normal sub-cap Medium/Hard compression untouched.
    hard_floor_chars = int(total_chars / MAX_COMPRESSION_RATIO)
    target_chars = max(target_tokens * CHARS_PER_TOKEN, hard_floor_chars)
    # Under pressure, prune past the target so following rounds are pure appends
    # (byte-stable prefix → provider cache hits) — but never below the cap floor.
    relief_chars = max(int(target_chars * PRESSURE_RELIEF), hard_floor_chars)
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # m12.1b (4): the ONE persistent break-prone signal kept from m12.1 — the same
    # failure recurring across recent results ("stuck on the same break") -> rich.
    # Depth-gated. (m12.1's shallow_small route + its 24/6/3 error guard are dropped;
    # they leaked Medium. Normal compressible Medium stays in harvest below.)
    repeated_fail = (
    ...
    if mode == "passthrough":
        ...
        # m12.1b (2): AGGRESSIVENESS CAP. Harvest's reduction is dominated by fixed
        # per-result truncation (MID_HEAD/MID_TAIL), not the budget, so on large inputs
        # it can still emit an ultra-small context (the 10-20x break-run regime). If the
        # harvest compressed its own input past MAX_COMPRESSION_RATIO, WIDEN the retained
        # context by switching to the gentle rich path (truncation-only, never drops,
        # much larger caps) — which provably stays well under the cap and preserves the
        # load-bearing context a flip/pass needs. Only genuinely over-aggressive harvests
        # (large contexts) trip this; normal sub-cap Medium/Hard harvests are untouched
        # (verified: a 5.45x Medium harvest is byte-identical to m12). The mode is set to
        # "rich" so the gentler path sticks for the rest of this (break-prone) session.
        harvest_in_tokens = final_token_estimate(working)
        harvest_out_tokens = final_token_estimate(result_messages)
        if harvest_out_tokens > 0 and harvest_out_tokens * MAX_COMPRESSION_RATIO < harvest_in_tokens:
            gentle_messages, gentle_info = compress_gently(working)
            gentle_info["reason"] = gentle_info.get("reason") or "gentle"
            gentle_info["capWidenedFromHarvest"] = True
            gentle_info["harvestRatio"] = round(harvest_in_tokens / harvest_out_tokens, 2)
            result_messages, info = gentle_messages, gentle_info
            mode = "rich"
        pruned = info.get("reason") in ("pruned", "gentle")
    ...
    # -----------------------------------------------------------------------
    # m12.1b (1a): NEVER-INFLATE GUARD (kept from m12.1) — final step before return,
    # for harvest AND rich (passthrough never compresses, so it is exempt by the
    # `changed` gate). Compare the FINAL output's token estimate to the BASELINE raw
    # input; if we did not actually save tokens (output >= raw, ratio < 1.0), discard
    # the compressed form and emit PASS-THROUGH (raw messages, changed=False). This
    # guarantees ratio >= 1.0 — the miner can never grow the context.
