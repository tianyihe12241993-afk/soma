#!/usr/bin/env python3
"""SOMA miner aow_lite — Append-Only-Window (harvest-only freeze-on-emit) on m12.
...
escalation), so the emitted prefix shifts turn-to-turn and the provider prompt-cache
misses. AOW-lite makes m12's stated intent EXPLICIT: it compresses each turn's content
ONCE when it ages out of a recent verbatim window, FREEZES those exact output bytes,
and REUSES them verbatim on later turns so the emitted prefix is byte-identical and
cache-friendly. The compression ENGINE is m12's, untouched.

SAFETY PRINCIPLE (overriding): in ANY ambiguous/edge/uncertain case AOW-lite falls
back to producing the BYTE-EXACT output m12 would produce — worst case == m12. The
freeze applies ONLY in harvest mode, ONLY before the session has ever shown rich
depth (everTouchedRich gate), and ONLY when our verified previous output is a
byte-identical prefix of the working set. Any prefix mismatch, orphan, ratio
regression, or rich-depth signal reverts the turn (and from then on the session) to
m12's exact harvest path. compress_gently (RICH path), passthrough, and
compress_structurally bodies are UNCHANGED; we only change WHAT we feed
compress_structurally in harvest and how we assemble its output around a frozen prefix.
...
# Bumped from m12's 4 -> 5: AOW-lite persists three additional state fields
# (frozenPrefix / frozenCount / everTouchedRich). A distinct version guarantees an
# AOW state file is never read by m12 and an m12 state file is never read by AOW
# (load_state rejects a version mismatch -> returns None -> fresh start -> exact m12
# first-turn behavior, no freeze). Cross-contamination is impossible by construction.
STATE_VERSION = 5
...
# AOW-lite recent verbatim window (NEW). The "live region" is the last WINDOW_MSGS
# *messages* of the assembled harvest output; everything older is eligible to be
# frozen. This window must be conservatively LARGER than what m12's harvest keeps
# untouched so the freeze never swallows content m12 would have kept fresh. m12's
# harvest protects the last TIGHT_KEEP_RECENT (=4) tool *results* (plus each one's
# invoking assistant tool-call), i.e. roughly 4 call/result pairs ≈ 8 messages of
# tail. We keep MORE than that verbatim: 24 messages (~12 recent interactions),
# comfortably above m12's untouched tail, so a freeze only ever covers content that
# is already well past m12's recent window. The frozen boundary is also SNAPPED BACK
# to a tool-call boundary (find_call_index) so we never freeze across a call/result
# pair. Larger window = safer (more recent verbatim, fewer bytes frozen, smaller
# blast radius); the cache win comes from the OLD prefix being byte-stable, which
# holds for any window >= m12's tail.
WINDOW_MSGS = 24
...
# ---------------------------------------------------------------------------
# AOW-lite: harvest-only freeze-on-emit assembly (NEW).
#
# Engages ONLY when the everTouchedRich gate (handle_assemble) allows it AND our
# verified previous output is a byte-identical prefix of `working`. Compresses ONLY
# the live region (working[frozenCount:]) with the UNMODIFIED compress_structurally,
# then assembles [*frozenPrefix, *compressed_live]. The frozen boundary for next turn
# is the recent verbatim WINDOW_MSGS window, ANCHORED (capped) to the longest prefix
# our output shares byte-for-byte with m12_native — so the frozen region is always
# exactly what m12 itself ships there. That anchor makes the freeze persist only while
# m12's own prefix is stable (the real append-only cache win) and regress to track m12
# the instant m12 churns/tightens the front, instead of thrashing the watchdog. The
# boundary is snapped back to a tool-call boundary so we never freeze across a
# call/result pair. Three independent guards each fall back to the byte-exact m12
# output (`m12_native`) for the turn: (a) prefix mismatch, (b) orphan introduction,
# (c) ratio worse than m12 (TIGHT +0.5% margin -> worst-case ratio == m12). Worst
# case == m12 on every turn.
# ---------------------------------------------------------------------------

def _common_msg_prefix_len(a: list[Any], b: list[Any]) -> int:
    """Number of leading messages that are BYTE-identical (canonical JSON) between
    `a` and `b`. Used to anchor the frozen boundary to the region where our assembled
    output already agrees with m12's freshly re-pruned output (m12_native): any prefix
    common to both is, by definition, exactly what m12 itself ships there, so freezing
    only that region can never make AOW larger than m12 in the frozen part, and it
    stays stable across turns precisely while m12's own prefix is stable."""
    n = min(len(a), len(b))
    k = 0
    while k < n and canonical_json(a[k]) == canonical_json(b[k]):
        k += 1
    return k
...
def assemble_aow_harvest(
    ...
    computed by the caller (with the REAL, sticky escalation). On any guard failure
    this function returns exactly (m12_native, m12_info, [], False) — a one-turn cache
    bust that ships exactly what m12 would, and re-seeds an empty freeze."""
    ...
    # Compress ONLY the live region with the UNMODIFIED compress_structurally. When
    # frozen_count == 0 this is m12's normal whole-working compress (turn 1 / re-seed).
    live_region = working[frozen_count:]
    compressed_live, live_info = compress_structurally(
        live_region, escalation, target_tokens=TARGET_TOKENS
    )
    result_messages = [*frozen_prefix, *compressed_live]
    ...
    # GUARD (c): ratio watchdog. The frozen prefix is never re-pruned, so when m12's
    # sticky escalation later TIGHTENS (trunc2/assistCap/tailCap flip) or m12 DROPS a
    # front interaction, m12_native shrinks that region while our frozen bytes stay at
    # the older, looser depth — [*frozenPrefix, *compressed_live] would then exceed what
    # m12 ships. The margin is TIGHT (RATIO_MARGIN, +0.5%) so AOW's worst-case ratio is
    # effectively == m12 (the previously-unguarded up-to-+5% harvest divergence can no
    # longer ship); the small epsilon only absorbs token-estimate granularity, never a
    # real regression. On any exceedance we DISCARD the AOW result and ship m12_native
    # (a one-turn cache bust that re-seeds the freeze), so AOW never ships a worse ratio
    # than m12 on ANY turn — including escalation-change / drop turns.
    aow_tokens = estimate_tokens(result_messages)
    m12_tokens = estimate_tokens(m12_native)
    RATIO_MARGIN = 1.005  # at most +0.5% over m12 (estimate noise only); else revert
    if aow_tokens > max(m12_tokens, 1) * RATIO_MARGIN:
        aow_info["aowFallback"] = "ratio_watchdog"
        aow_info["aowTokens"] = aow_tokens
        aow_info["m12Tokens"] = m12_tokens
        return m12_native, {**m12_info, **aow_info}, [], False

    # Engaged. Advance the freeze for NEXT turn. The window-based boundary keeps the
    # last WINDOW_MSGS messages verbatim-live, then two SAFETY CAPS are applied:
    #
    #  CAP 1 (correctness, REQUIRED): the frozen prefix must NOT contain the FIRST USER
    #  message. compress_structurally locates the first user message in its input and
    #  no-ops ("missing_first_user_message") if there is none — so if the freeze ever
    #  swallowed the first user message, next turn's live region would lose it and ship
    #  UNCOMPRESSED (a large ratio regression). m12's engine is built around a fixed
    #  head (leading system + first user message) that it always keeps byte-stable and
    #  prunes only the middle; the cache-stable prefix m12 already delivers IS that head.
    #  So the freeze is capped at first_user_index: it can lock the leading system head
    #  (which m12 keeps byte-stable anyway) but never reaches into the prunable middle.
    #  This is the architectural ceiling the verification proved — in the harvest path
    #  AOW degrades to byte-exact m12, which is the safe, correct behavior (worst == m12).
    #
    #  CAP 2 (anchor): also cap to the longest prefix our output shares BYTE-FOR-BYTE
    #  with m12_native, so the frozen bytes are exactly what m12 itself ships there
    #  (defense-in-depth: AOW can never exceed m12 in the frozen region, and the freeze
    #  regresses to track m12 the instant m12 churns the front).
    #
    # The boundary is NOT clamped monotonic (it may regress to track m12), then snapped
    # back to a tool-call boundary so a call/result pair is never split.
    first_user_index = next(
        (
            i
    ...
    window_boundary = len(result_messages) - WINDOW_MSGS
    m12_common = _common_msg_prefix_len(result_messages, m12_native)
    raw_boundary = max(0, min(window_boundary, m12_common, first_user_index))
    new_boundary = _snap_freeze_boundary(result_messages, raw_boundary)
    new_boundary = max(0, min(new_boundary, first_user_index))
    next_frozen_prefix = result_messages[:new_boundary]

    aow_info.update(
        {
            "aowEngaged": True,
            "aowFrozenCountIn": frozen_count,
            "aowFrozenCountOut": new_boundary,
            "aowLiveRegionLen": len(live_region),
