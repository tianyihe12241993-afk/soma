...
# uphard = HARD-ZONE specialist (PROPORTIONAL fork of np2). THESIS (2026-06-29 per-task re-analysis +
# Codex audit, reports/reanalysis_2026-06-29.md): Hard flips need CONTEXT, so a too-aggressive flat cap
# HURTS Hard (verified: np2 @16k -> H0.173 < np3 @28k -> H0.369; fuller=more flips). The Hard-zone winner
# 5GgVXz (H0.676, Pair(E,H) 0.760) keeps BIG results FULLER (~1.68x) than np2's flat cap (which hammers a
# 40k result to 16k = 2.5x, losing flip-context) WHILE compressing the small/mid results np2 passes
# through (for aggregate savings). A flat char cap CANNOT do that (its ratio scales with result size); a
# PROPORTIONAL cap (keep a fixed FRACTION of each result) gives the uniform ~1.7x 5GgVXz profile and is
# still PURE/DETERMINISTIC per-message (cap depends only on THIS message's own length) -> cache-stable, and
# IDEMPOTENT via the CMP_START skip (re-fed [[CMP]] output is untouched). NO-INFLATION guard inherited ->
# bounded. NOTE: this targets Pair(E,H)+Single-H (the 14.3% zone, own sacrificial hotkey, cannot hurt our
# live floor); Medium may break (irrelevant to those elements). Codex caught an earlier flat-4k version as
# BACKWARDS (over-compressed -> killed flips); this is the corrected direction.
NP_KEEP_FRAC = 0.60            # keep ~60% of each engaged result -> ~1.67x; fuller big results -> Hard flip-context.
NP_MIN_FLOOR = 16_000          # results <= this PASS THROUGH = IDENTICAL to np2's 16k cap -> preserves Easy 1.052 + Medium 0.849
                               # (the COMBINED-miner thesis, reports/reanalysis_2026-06-29.md: np2's E+M + np3's Hard -> Overall ~0.76 > king 0.728).
NP_MAX_KEEP = 32_000           # UPPER cap on the proportional keep: a HUGE result keeps min(0.60*len, 32k) -> stays in np3's PROVEN
                               # Hard zone (np3 flat 28k -> H0.369), NOT ballooning to 60k+ which risks np5's near-passthrough crater
                               # (np5 flat 48k -> 0.25). Net per-result map: <=16k passthrough(=np2) | 16-26.7k cap 16k(=np2) |
                               # 26.7-53.3k keep 0.60(16-32k, fuller) | >53.3k cap 32k(~np3). So uphard = np2 on E/M results, ~np3 on huge Hard results.
NP_RESULT_CAP = NP_MIN_FLOOR   # alias kept for the metadata field below; the EFFECTIVE cap is the bounded-proportional value (see cap_tool_result).
...
# ============================================================================================
# ⚠️⚠️ OMITTED-COUNT ANNOTATION — GATED, OFF BY DEFAULT, NOT YET COMPLIANT ⚠️⚠️
# Karim's proposal (Discord 2026-06-29 15:05): an INFORMATIVE marker stating how many chars
# were omitted, e.g. "[[CMP]] {n} chars omitted [[/CMP]]". Matt|SOMA: "Looks okay to me at first
# glance" (his FAVORED 'informative-only' category) BUT it is NOT yet in miner/README_prompting.md,
# so emitting it NOW = FAIL REVIEW. This variant is PRE-BUILT and DORMANT:
#   - OMIT_COUNT_ENABLED = False  -> this file is BYTE-BEHAVIORALLY IDENTICAL to uphard_cap32 (COMPLIANT).
#   - Flip to True ONLY AFTER: (1) the exact string lands in README_prompting.md, (2) you set
#     OMIT_COUNT_TEMPLATE to the README's EXACT wording, (3) `python scripts/check_readme_current.py`
#     confirms it, (4) scanner + Codex re-audit. Until all 4, LEAVE FALSE. DO NOT UPLOAD WITH IT TRUE.
# Why it could help us: telling Qwen3 that content was truncated -> it stops assuming it has the full
# file -> fewer breaks / better Hard flips (directly aids uphard's combined-Overall goal). Informative,
# not directive -> the category SOMA will allow if any.
OMIT_COUNT_ENABLED = False         # <-- KEEP FALSE until Karim's marker lands in README_prompting.md (see above)
OMIT_COUNT_TEMPLATE = "{n} chars omitted"   # <-- placeholder; MATCH the README's EXACT approved string before enabling
# ============================================================================================
...
def cap_tool_result(message, cap):
    ...
    if normalize_role(message.get("role")) != "toolResult":
        return message, False
    text = extract_text(message.get("content"))
    if CMP_START in text:  # IDEMPOTENCY guard FIRST (Codex re-audit): already-compressed result is a fixed point,
        return message, False  # returned UNCHANGED before any cap math -> emitted prefix byte-stable turn-to-turn.
    # PROPORTIONAL cap (uphard): keep ~NP_KEEP_FRAC of THIS result, with a passthrough floor. Pure function
    # of this message's OWN length only (no trajectory/recency) -> byte-stable turn-to-turn = cache-safe.
    # Big results stay FULLER than a flat cap (keep 60% of 40k = 24k, vs np2's flat 16k) -> Hard flip-context
    # preserved; small/mid results get compressed for savings. `cap` arg ignored in favour of the proportional value.
    cap = min(NP_MAX_KEEP, max(NP_MIN_FLOOR, int(len(text) * NP_KEEP_FRAC)))
    if len(text) <= cap:
        return message, False
    inner_cap = max(256, cap - len(CMP_START) - len(CMP_END) - 2)
    inner, _changed = extractive_compress(text, inner_cap, frozenset())
    if OMIT_COUNT_ENABLED:  # GATED (see top): append the informative omitted-char count INSIDE the [[CMP]] block.
        omitted = len(text) - len(inner)  # chars dropped by the extractive cut
        if omitted > 0:
            inner = f"{inner}\n{OMIT_COUNT_TEMPLATE.format(n=omitted)}"
