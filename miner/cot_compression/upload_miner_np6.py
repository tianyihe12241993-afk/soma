...
NP_RESULT_CAP = 16_000         # np2: LIGHTER cap than np1/m26's 6k. m26 scored #3 (0.610) but its 2 worst losses were
                               # BREAKS on its 2 HARDEST-compressed tasks (315 @2.43x, 297 @2.86x) — over-compressing big
                               # tool results lost info the agent needed. #1 (5DCnA57) wins by being LIGHT (1.18x, fewer
                               # breaks). m26 already has BETTER cache (93% vs 91%) + tied Easy, so the only fix is LIGHTER:
                               # keep big critical results fuller -> fewer breaks/wander, toward 5DCnA57's ~1.2x sweet spot.
                               # Flat cap (idempotent -> cache-stable; a fractional/recency cap would multi-step-shrink ->
                               # churn the prefix). 16k = first cut (target ~1.3x, between m26's 1.64x and #1's 1.18x, safely
                               # above 5HdTr7's 1.07x inflate-trap); the PLATFORM calibrates (too light->inflate-trap; too
                               # heavy->breaks). Mechanism otherwise identical to np1 (cache-stable inherited).
...
# np6 HYBRID anchor selection (Codex design-review change, reports/cache_stable_design.md §20). The plain
# line-based extractive_compress above keeps function SIGNATURES and drops BODIES -> a table-of-contents
# skeleton (166-221 "…" gaps on real 64k break blocks). The hybrid keeps the single highest-confidence
# load-bearing UNIT coherent, then fills the rest with line-based breadth, falling back to np2 behavior when
# there is no clear anchor or it would blow the budget (Codex: pure-coherent risks DROPPING a critical unit).
_ANCHOR_RE = re.compile(
    r"Traceback|AssertionError|\bError\b|\bException\b|^E\s|FAILED|^\s*assert\b|^@@|^[-+]{3}\s"
)


def _find_anchor(lines):
    """Index of the FIRST strong load-bearing line (error/traceback/assertion/test-failure/diff hunk),
    or None if the result has no clear anchor (then fall back to plain line-based extraction)."""
    for i, line in enumerate(lines):
        if line.strip() and _ANCHOR_RE.search(line):
            return i
    return None


def _anchor_window(lines, anchor, budget):
    """Coherent CONTIGUOUS window around the anchor: expand to blank-line boundaries both ways, then shrink
    the far ends toward the anchor until it fits `budget` chars. Keeps the load-bearing region WHOLE (vs
    scattering its lines). Robust by design — no fragile indent/structure parsing (Codex brittleness flag)."""
    n = len(lines)
    start = anchor
    ...
    while size(start, end) > budget and (start < anchor or end > anchor):
        if (anchor - start) >= (end - anchor) and start < anchor:
            start += 1
        elif end > anchor:
            end -= 1
        else:
...
def extractive_compress_hybrid(text: str, target_chars: int, active: frozenset = frozenset()) -> tuple[str, bool]:
    """np6 HYBRID: keep the single highest-confidence load-bearing UNIT COHERENT (contiguous — the error/
    traceback/failing-test/diff region the agent must actually read), then spend the remaining budget on
    np2's line-based breadth (the breadcrumbs that carry E/M). FALLS BACK to plain extractive_compress when
    there is no clear anchor OR the anchor alone would exceed half the budget. Fixes the signatures-without-
    bodies skeleton (§20) without sacrificing breadth. PURE + DETERMINISTIC + IDEMPOTENT (same guarantees as
    extractive_compress: depends only on this message's own content; cap_tool_result's [[CMP]] skip + no-
    inflation guard make the emitted prefix byte-stable turn-to-turn)."""
    if not isinstance(text, str) or len(text) <= target_chars:
        return text, False
    lines = text.split("\n")
    n = len(lines)
    ...
    if anchor is None:
        return extractive_compress(text, target_chars, active)          # no anchor → exact np2 behavior
    a_start, a_end = _anchor_window(lines, anchor, int(target_chars * 0.4))
    keep = set(range(a_start, a_end + 1))
    used = sum(len(lines[i]) + 1 for i in keep)
    if used > target_chars * 0.5:
        return extractive_compress(text, target_chars, active)          # anchor too big → exact np2 behavior
    # breadth: pin load-bearing lines OUTSIDE the anchor, then top-TF-IDF lines, within the remaining budget
    for i in range(n):
        if i not in keep and lines[i].strip() and _line_is_pinned(lines[i], active):
            keep.add(i)
            used += len(lines[i]) + 1
    remaining = [i for i in range(n) if i not in keep and lines[i].strip()]
    if used < target_chars and remaining:
    ...
    if prev < n - 1:
        out.append("…")
    result = "\n".join(out)
    if len(result) >= len(text):  # extraction didn't help → fall back to truncation (same as np2)
        return truncate_text(text, int(target_chars * 0.75), int(target_chars * 0.25)), True
    return result, True
...
def cap_tool_result(message, cap):
    ...
    inner, _changed = extractive_compress_hybrid(text, inner_cap, frozenset())
