...
def count_tokens(text: str) -> int:
    """Deterministic LOCAL token count (tiktoken cl100k proxy — NOT an LLM/API call). Pure fn of the
    text, so the per-message cap decision stays deterministic/idempotent -> cache-stable. Falls back to
    a char/CHARS_PER_TOKEN estimate if tiktoken is unavailable."""
...
# nptok: TOKEN-based cap (vs np2's char-based NP_RESULT_CAP). The score weighs TOKENS not chars, and
# tiktoken counts diverge from char-length by content density (code/JSON/traces tokenize denser than
# prose). 4096 tok ~= np2's 16k chars at ~3.9 chars/tok; the EXPERIMENT is the BASIS (token vs char),
# which fires on a different set of results. Validated locally before any hotkey is spent.
NP_TOKEN_CAP = 4096
...
def cap_tool_result(message, cap):
    ...
    if normalize_role(message.get("role")) != "toolResult":
        return message, False
    text = extract_text(message.get("content"))
    # TOKEN-BASED cap (nptok variant): decide + target on tiktoken token count, not chars. `cap` is a
    # TOKEN budget. Idempotent (already-[[CMP]] skipped) + deterministic (token count is a pure fn of
    # the content) -> same cache-stability guarantee as np2. Compliant: tiktoken is a LOCAL tokenizer,
    # not an LLM/API call, and the decision uses ONLY this result's own bytes (no task/category signal).
    n_tok = count_tokens(text)
    if CMP_START in text or n_tok <= cap:
        return message, False
    # target ~`cap` tokens; convert to a char budget via THIS result's own density (chars per token).
    cpt = len(text) / max(1, n_tok)
    target_chars = max(256, int(cap * cpt))
    inner_cap = max(256, target_chars - len(CMP_START) - len(CMP_END) - 2)
    inner, _changed = extractive_compress(text, inner_cap, frozenset())
    wrapped = cmp_block(inner)
    # NO-INFLATION INVARIANT (np2's char guard + a NEW token guard): emit the capped/wrapped form ONLY
    # if it is STRICTLY SMALLER in BOTH chars AND tokens. The token guard closes the char-vs-token
    # fragmentation trap (m21/m22): extractive output can tokenize higher per char, so a char-only guard
    # could still inflate TOKENS (what the score weighs). With both, output <= native in chars and tokens.
    if len(wrapped) >= len(text) or count_tokens(wrapped) >= n_tok:
        return message, False
    out = copy.deepcopy(message)
    out["content"] = wrapped
    return out, True
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for message in working:
        capped, changed = cap_tool_result(message, NP_TOKEN_CAP)
