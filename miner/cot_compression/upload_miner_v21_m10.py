...
# v21 OLD-REASONING COMPRESSION. The agent's own narration ("let me check X…")
# accumulates across 40-70 turns (see the django-14007 48-step trajectory) and is
# noise IT generated, not tool signal. Trimming OLD reasoning (keep head+tail =
# intent + conclusion, drop the verbose middle) lowers comp_ratio -> more savings
# bonus on EVERY passing run (Easy/Medium/Hard-bonus), without removing facts the
# agent needs. Tool calls + tool results + recent reasoning stay byte-intact, so
# pairing is preserved and the agent's working memory is untouched. Pure
# compression (compliant). Unlike v14/v17/v19/v20 it touches the agent's noise,
# not the tool-result signal that every prior lever wandered on.
REASONING_INTACT_MSGS = 12     # keep the last N messages' reasoning byte-intact
REASONING_HEAD = 300           # chars kept from the start of an old reasoning block (intent)
REASONING_TAIL = 300           # chars kept from the end (conclusion / decision)
...
def _trim_reasoning_text(text: str) -> tuple[str, bool]:
    if not isinstance(text, str) or len(text) <= REASONING_HEAD + REASONING_TAIL + 40:
        return text, False
    return text[:REASONING_HEAD] + "\n…[soma: reasoning trimmed]…\n" + text[-REASONING_TAIL:], True


def compress_old_reasoning(messages: list[Any]) -> tuple[list[Any], int]:
    """v21: trim verbose OLD assistant reasoning (keep head+tail), leaving tool
    calls, tool results, user/system, and the last REASONING_INTACT_MSGS messages
    untouched. Returns (messages, trimmed_count)."""
    n = len(messages)
    cutoff = max(0, n - REASONING_INTACT_MSGS)
    out: list[Any] = []
    trimmed = 0
    for i, m in enumerate(messages):
        if i >= cutoff or not isinstance(m, dict) or normalize_role(m.get("role")) != "assistant":
            out.append(m)
            continue
        content = m.get("content")
        if isinstance(content, str):
            new, ch = _trim_reasoning_text(content)
            if ch:
                mm = copy.deepcopy(m); mm["content"] = new; out.append(mm); trimmed += 1
            else:
                out.append(m)
            ...
            for b in content:
                if (isinstance(b, dict) and b.get("type") in (None, "text")
                    ...
                    new, ch = _trim_reasoning_text(b["text"])
                    if ch:
                        nb = copy.deepcopy(b); nb["text"] = new; new_blocks.append(nb); ch_any = True
                    else:
                        new_blocks.append(b)
                else:
                    new_blocks.append(b)  # toolCall and other blocks untouched
            if ch_any:
                mm = copy.deepcopy(m); mm["content"] = new_blocks; out.append(mm); trimmed += 1
            else:
                out.append(m)
        else:
            out.append(m)
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # v21: trim OLD verbose reasoning (assistant text only). Feeds all compressing
    # modes (passthrough ignores `working`, so its prompt-cache path is untouched).
    working, reasoning_trimmed = compress_old_reasoning(working)
    ...
    metadata = {
        ...
        **info,
        ...
        "reasoningTrimmed": reasoning_trimmed,
        "mode": mode,
