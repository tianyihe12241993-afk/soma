#!/usr/bin/env python3
"""SOMA miner m25 — m12 (m7_compliant) + GATED GENTLE BREAK-FIX + DECOUPLED STATE.

m25 = m12 with three surgical, compliance-preserving additions, all confined to the HARVEST
path. rich/compress_gently, passthrough, the router dispatch, save/load plumbing shape, and
every helper are byte-identical to m12 except where noted:
  (1) GENTLE BREAK-FIX (m17-style): at TRUNCATION sites only, keep the load-bearing span
      (error/traceback/failing-test/diff/signature/path lines, via m12's own extractive_compress)
      instead of blind head/tail truncation. DROPS are left EXACTLY as m12 drops them (no
      drop-spans — the m22 lever that over-destabilized). Worst case per result == m12.
  (2) RESOLVED-GATE: the break-fix is applied ONLY on turns whose working set is "settled"
      (objective signals m12 already computes: recent results error-free, not still-failing,
      not escalated, not deep, prev mode not rich). Every other turn's harvest output is
      byte-identical to m12, keeping break-fix off actively-failing / flip phases.
  (3) DECOUPLED STATE: when the break-fix is applied we EMIT the break-fixed harvest (what the
      model sees) but SAVE the m12-EXACT harvest to our state file. Under the native-feed
      (source-match) connector, the next turn's working set — and thus rich/Hard — reads the
      clean m12 content, so a fired gate on a to-become-rich task cannot drift into a Hard
      crater. Under output-match the saved-clean is unused (no worse than m12 + gentle break-fix).
A RECONCILIATION guard ships the break-fix build only if it is <= the m12 build in BOTH tiktoken
tokens AND raw chars; else it falls back to the m12-exact build. So m25 never puts more scored
tokens before the model than m12, and on any non-resolved turn (or reconciliation miss) m25 is
m12 byte-for-byte (output AND saved state). All markers / loop-detection are m12's allowed set;
no new strings, no steering, no task/category awareness, no LLM calls. SEPARATE-HOTKEY candidate;
the live m12 (upload_miner_m7_compliant.py) is never touched.

--- original m12 header below (compression ENGINE unchanged) ---
...
STATE_VERSION = 7  # m25: state schema adds decoupled clean-messages semantics (own hotkey; no conflict with m12 v4)
STATE_DIR_NAME = "state"

# ── m25 RESOLVED-GATE thresholds (objective, compression-state-only signals) ──
# The break-fix is applied ONLY when ALL of these hold (else harvest == m12 exactly):
M25_GATE_ERR_WINDOW = 6   # the recent tool-results must be error-free over this many
M25_GATE_MAX_DEPTH = 80   # stay clear of the rich-transition zone (LARGE_THRESHOLD_MSGS=90)
M25_GATE_MIN_RESULTS = 3  # need some older results for a break-fix to matter
...
def save_state(
    ...
    *,
    ...
    clean_messages: list[Any] | None = None,
) -> None:
    # m25 DECOUPLING: `output_messages` is what we RETURNED to the model (break-fixed on a
    # resolved harvest turn); `clean_messages` is what we PERSIST as the working history
    # (the m12-exact build). The output fingerprint/count describe the RETURNED output, so the
    # output-match (connector-rewrite) path still loads correctly; but `messages` stores the
    # clean build, so the source-match path feeds rich/Hard the m12-exact content next turn.
    # clean_messages is None for every non-decoupled turn -> identical to m12 (stores output).
    if clean_messages is None:
        clean_messages = output_messages
    ...
    state = {
        ...
        "messageCount": len(clean_messages),
        "mode": mode,
...
def _harvest_build(
    ...
    *,
    ...
    """m12's harvest engine, parameterized by `use_extractive`.

    use_extractive=False  -> byte-identical to m12.compress_structurally (blind head/tail
                             truncation at kept-truncation sites; drops removed to 0 chars
                             with their toolCall stripped). The clean/legacy build.
    use_extractive=True   -> m25 GENTLE BREAK-FIX: at the SAME truncation sites, keep the
                             load-bearing span via extractive_compress THEN apply the SAME
                             head/tail cap (so bytes <= the blind truncate in every case).
                             DROPS are unchanged (no drop-spans). The DROP/TRUNC classification,
                             escalation lever order, pairing, and planned_chars are m12-identical;
                             only the per-result REDUCTION of kept-truncated results differs."""
    ...
    harvest_active = frozenset()  # pure function of message text -> cache-stable; m25 pins on universal signals

    def _reduce(msg: Any, ex_budget: int, cap_head: int, cap_tail: int) -> tuple[Any, bool]:
        # m25 GENTLE BREAK-FIX: extractive SELECTION (keep error/test/diff/sig/path lines) then the
        # SAME hard head/tail CAP m12 used, so bytes are <= m12's blind truncate in every case
        # (extraction shrinks line-structured results -> the cap is a no-op and the buried clue
        # survives; un-shrinkable blobs fall through to the identical m12 truncate). When
        # use_extractive=False this is EXACTLY m12's truncate_message (byte-identical legacy build).
        if use_extractive:
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult":
            ...
            if index in truncated_protected:
                ...
                message, _ = _reduce(message, TAIL_RESULT_CAP, int(TAIL_RESULT_CAP * 0.75), int(TAIL_RESULT_CAP * 0.25))
            elif index not in untouchable:
                message, _ = _reduce(message, ABS_RESULT_CAP, int(ABS_RESULT_CAP * 0.75), int(ABS_RESULT_CAP * 0.25))
            output.append(message)
            continue
        if role == "assistant":
...
def compress_structurally(
    ...
    """m25 harvest entry. Returns (OUTPUT, info, CLEAN):
      - OUTPUT   = what we return to the model (gentle break-fix when `resolved` AND it
                   reconciles <= m12 on tokens AND chars; otherwise the m12-exact build).
      - CLEAN    = the m12-exact build, ALWAYS — persisted to state.messages so the
                   source-match (native-feed) path feeds rich/Hard the clean trajectory.
    On a non-resolved turn (or a reconciliation miss) OUTPUT is CLEAN (m12 byte-for-byte).
    The escalation flags committed to state are ALWAYS the m12/legacy flags, so the
    sticky-escalation contract is identical to m12 regardless of the break-fix attempt."""
    escalation = escalation if escalation is not None else {}

    # m12-exact (legacy) build: the clean state, the budget ceiling, and the fallback.
    legacy_esc = dict(escalation)
    legacy_out, legacy_info = _harvest_build(messages, legacy_esc, target_tokens, use_extractive=False)
    # Commit the m12 escalation flags (stickiness identical to m12; the break-fix build must
    # not be allowed to inflate them).
    escalation.clear()
    escalation.update(legacy_esc)

    if not resolved:
        return legacy_out, legacy_info, legacy_out

    # Resolved turn: try the gentle break-fix; ship ONLY if <= m12 in BOTH tokens AND chars.
    bf_esc = dict(legacy_esc)
    bf_out, bf_info = _harvest_build(messages, bf_esc, target_tokens, use_extractive=True)
    legacy_chars = sum(message_cost(m) for m in legacy_out)
    legacy_tokens = final_token_estimate(legacy_out)
    bf_chars = sum(message_cost(m) for m in bf_out)
    bf_tokens = final_token_estimate(bf_out)
    if bf_tokens <= legacy_tokens and bf_chars <= legacy_chars:
        bf_info["m25"] = {
            "breakFixShipped": True, "legacyTokens": legacy_tokens, "bfTokens": bf_tokens,
            "legacyChars": legacy_chars, "bfChars": bf_chars,
        }
        return bf_out, bf_info, legacy_out  # DECOUPLE: emit break-fix, persist clean=legacy
    legacy_info["m25"] = {
        "breakFixShipped": False, "reason": "reconcile_miss",
        "legacyTokens": legacy_tokens, "bfTokens": bf_tokens,
        "legacyChars": legacy_chars, "bfChars": bf_chars,
    }
    return legacy_out, legacy_info, legacy_out
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # m25 RESOLVED-GATE: the gentle break-fix is applied ONLY on a "settled" harvest turn —
    # recent results error-free, not still-failing, not escalated, not deep, prev mode not rich.
    # These are the SAME objective compression-state signals m12 already computes (no task/
    # category/benchmark awareness). On every other turn harvest is byte-identical to m12.
    m25_resolved = (
        mode == "harvest"
        and prev_mode != "rich"
        and not still_failing
        and not (escalation.get("assistCap") or escalation.get("trunc2") or escalation.get("tailCap"))
        and msg_depth < M25_GATE_MAX_DEPTH
        and recent_errors(working, M25_GATE_ERR_WINDOW) == 0
        and sum(1 for m in working if isinstance(m, dict) and normalize_role(m.get("role")) == "toolResult") >= M25_GATE_MIN_RESULTS
    )

    sanitize_changed = False
    pruned = False
    clean_messages: list[Any] | None = None  # m25: what we PERSIST (m12-exact); None => == output
    if mode == "passthrough":
        ...
        sanitized = working
        # m25: compress_structurally returns (OUTPUT, info, CLEAN). OUTPUT is the gentle
        # break-fix when the gate fired AND it reconciled <= m12; else m12-exact. CLEAN is
        # ALWAYS the m12-exact build, persisted to state so rich/Hard reads clean next turn.
        result_messages, info, clean_messages = compress_structurally(
            working, escalation, target_tokens=TARGET_TOKENS, resolved=m25_resolved
        )
        pruned = info.get("reason") == "pruned"
        # Orphan guard is checked on the OUTPUT we will actually ship; the clean build shares
        # the identical drop/pairing structure so it is guarded by the same check.
        ...
        if not (out_result_orphans <= in_result_orphans and out_call_orphans <= in_call_orphans):
            ...
            clean_messages = sanitized  # keep persisted state consistent with the abandoned output
            info = {"reason": "orphan_guard_fallback"}
            pruned = False
        if not result_messages:
            result_messages = raw_messages
            clean_messages = raw_messages
            info = {"reason": "empty_output_fallback"}
            pruned = False
        # Compliant loop guard: appends ONLY an allowed loop-reason string, and only
        # when an objective repeated/no-progress loop is detected. No steering. (Applied to
        # the emitted OUTPUT only; the loop guard is stripped on load, so CLEAN need not carry it.)
    ...
    # m25: for passthrough/rich (and any harvest fallback that set it) clean == output exactly,
    # so the persisted state matches m12 unless the decoupled break-fix actually shipped.
    if clean_messages is None:
        clean_messages = result_messages
    metadata["m25BreakFixShipped"] = bool(info.get("m25", {}).get("breakFixShipped")) if isinstance(info.get("m25"), dict) else False
    metadata["m25Resolved"] = m25_resolved
    try:
        save_state(
            state_path,
            payload,
