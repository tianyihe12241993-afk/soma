...
# v13: harvest target 8k -> 5k. Easy tokens are ~per-call (input ~target + ~3k
# uncompressible agent output) x call-count. We can't cut calls compliantly, but
# a leaner per-call cuts total ~30-40% across ALL runs (wander included), pulling
# easy from ~247k toward the king's 40-200k band -> real Easy bonus. The error
# guard still routes stuck hard-pass tasks (sympy) to RICH, so 5k only bites the
# genuinely-shallow/easy/medium harvest tasks. Validated: easy must drop AND stay
# gold AND medium/sympy must hold, else revert to 8k.
TARGET_TOKENS = 5_000          # v13 aggressive harvest target (was 8k)
...
# v12: detect a CLEARLY-PASSING test run so the coach can fire a compliant
# forced-stop at that exact stopping condition (cuts the post-fix exploration
# tail that inflates easy-task call count — our biggest gap vs the king). Pass
# requires a success signature AND no genuine-failure signature. The fail
# pattern deliberately ignores "0 failed"/"0 errors" (those co-occur with a pass).
TEST_PASS_PATTERN = re.compile(
    r"\b\d+\s+passed\b"                 # pytest "N passed"
    r"|^\s*OK\s*$"                      # unittest trailing "OK"
    r"|Ran\s+\d+\s+tests?[\s\S]{0,40}?\bOK\b"  # unittest "Ran N tests ... OK"
    r"|\ball\s+tests?\s+pass(?:ed)?\b"
    r"|\b0\s+failed\b.*\b0\s+errors?\b",
    re.M | re.I,
)
TEST_FAIL_PATTERN = re.compile(
    r"\b[1-9]\d*\s+failed\b"            # "1 failed" (not "0 failed")
    r"|\b[1-9]\d*\s+errors?\b"          # "2 errors"
    r"|\bFAILED\b"                      # pytest FAILED
    r"|\bTraceback\b"
    r"|\bAssertionError\b"
    r"|^\s*(?:FAIL|ERROR):",            # unittest FAIL:/ERROR:
    re.M,
)
...
def tests_passing(messages: list[Any]) -> bool:
    """True iff the most recent test-bearing tool result CLEARLY passed — a
    success signature present and no genuine-failure signature. Used to fire the
    forced-stop at the exact stopping condition (tests green). Looks only at the
    most recent test-bearing result so a stale early pass can't trigger it after
    the agent has changed things again."""
    for message in reversed(messages):
        if not isinstance(message, dict) or normalize_role(message.get("role")) != "toolResult":
            continue
        text = extract_text(message.get("content"))
        has_pass = bool(TEST_PASS_PATTERN.search(text))
        has_fail = bool(TEST_FAIL_PATTERN.search(text))
        if not (has_pass or has_fail):
            continue  # not a test-bearing result; look further back
...
def build_coach_text(messages: list[Any], *, governor: bool) -> str:
    ...
    # Forced stopping. v12: when tests are CLEARLY passing (a detected stopping
    # condition), fire a FIRM stop — this is what cuts the post-fix exploration
    # tail that inflates easy-task call count. Otherwise the generic gentle form.
    # Both are pure forced-stopping (no workflow-steering), within the ruling.
    if tests_passing(messages):
        parts.append(
            "Your tests are now passing and the change appears complete. STOP NOW: "
            "return your final answer. Continuing to explore or re-verify will not "
            "improve the result."
        )
    else:
        parts.append(
