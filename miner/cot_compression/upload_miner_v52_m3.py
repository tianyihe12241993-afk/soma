...
def build_coach_text(messages: list[Any], *, governor: bool) -> str:
    ...
    # (c) Forced stopping: name the stopping condition (which tests must pass),
    # then state the stop instruction.
    status = extract_test_status(messages)
    if status:
        parts.append("Stopping condition — the task is complete when these tests pass:")
        parts.append(status)
    parts.append(
        "When your fix is complete and the relevant tests pass, stop and return your "
        "final answer instead of continuing to explore."
    )
