...
def compress_structurally(
    ...
    for item in interactions:
        if item["duplicate"] and item["droppable"]:
            item["mode"] = "drop"
    # m19 CACHE-STABLE: truncation depth is FROZEN (no trunc2/assist_cap/tail_cap
    # escalation). An emitted message's compressed form is a pure function of its own
    # content at a FIXED depth, so it never re-morphs across turns. Pressure is
    # absorbed by DROPPING oldest droppable interactions to target (m12's primary,
    # monotonic lever), NOT by repeatedly deepening truncation on already-emitted
    # messages. trunc2 (the dominant re-morph source) is removed entirely.
    assist_cap = trunc2 = tail_cap = False
    if planned_chars(assist_cap, trunc2, tail_cap) > target_chars:
        # Sacrifice unprotected interactions first (oldest first), protected
        # (error/test output) only if the budget still demands it. Drop all the way
        # to TARGET (m12 stopped at relief then deepened truncation; m19 keeps dropping).
        for item in sorted(interactions, key=lambda it: (it["protected"], it["result_index"])):
            if planned_chars(assist_cap, trunc2, tail_cap) <= target_chars:
                break
            if item["droppable"]:
                item["mode"] = "drop"
    escalation["assistCap"] = escalation["trunc2"] = escalation["tailCap"] = False
    ...
    # m19: ALL depths FIXED (no escalation) so every emitted form is byte-stable.
    head, tail = (MID_HEAD, MID_TAIL)            # frozen; never MID2
    untouchable = set(result_indices[-TIGHT_UNTOUCHABLE:])   # last 2 results: byte-intact
    tail_results = set(result_indices[-TIGHT_KEEP_RECENT:])  # recency band: kept FULLER, but FIXED
    output: list[Any] = []
    for index, message in enumerate(messages):
        role = normalize_role(message.get("role")) if isinstance(message, dict) else ""
        if role == "toolResult":
            ...
            if index in truncated_protected:
                ...
                info["truncatedResultCount"] += int(did)
            elif index in untouchable:
                pass  # last 2 results: byte-intact
            elif index in tail_results:
                # recency band (positions -3,-4): kept fuller at a FIXED cap (no tail_cap
                # conditionality) -> stable form, preserves recent context for the agent.
                message, _ = truncate_message(message, int(TAIL_RESULT_CAP * 0.75), int(TAIL_RESULT_CAP * 0.25))
            else:
                # everything older: FIXED ABS cap (no escalation) -> stable; the harvest's
                # aggressiveness comes from DROPS, not from re-deepening this cap.
        ...
        if role == "assistant":
            if index in strip_by_call:
                ...
                if assistant_is_empty(message):
                    continue
            # m19: assistants are NOT truncated (no assist_cap escalation) -> stable;
            # only tool-call stripping (from drops) touches them, which is monotonic.
            output.append(message)
            continue
        output.append(message)
