#!/usr/bin/env python3
"""SOMA miner m16_small_context_light_safe — m12 (upload_miner_m7_compliant.py) + ONE scoped change.

THE ONLY change vs m12: a SMALL-CONTEXT LIGHT-SAFE RESCUE inside the HARVEST branch. After harvest, if the
harvest output did NOT actually save tokens (output >= raw input — the small-context INFLATION signature; on
comp-108 m12 inflates ~6/15 small Easy tasks → score penalty), replace the inflated harvest with the GENTLE
RICH path (light, load-bearing-preserving, allowed BLOCK/Same-response markers). Use rich only if it gives
POSITIVE savings (ratio>1) — never pass-through as the strategy; only fall to raw as a last resort when nothing
is safely compressible (never inflate). The mode stays "harvest" (NON-STICKY) so a growing Medium/Hard task is
never trapped in rich (the m13 mistake).

WHY REACTIVE (harvest-didn't-save) not a PROACTIVE small-context threshold: M/H harvests always save (ratio
1.5-4x), so this trigger PROVABLY never fires on them → Medium/Hard byte-identical (enforced by offline checks).
A proactive threshold would catch Medium/Hard in their early-small rounds and mode-stickiness would trap them in
rich → the m13 Medium collapse. This trigger is self-targeting to exactly the small/incompressible Easy band.

DELIBERATELY EXCLUDED (rejected designs): NO m13 aggressiveness cap / MAX_COMPRESSION_RATIO, NO harvest>8x->rich
fallback (m13's trigger is over-compression; THIS is non-savings — opposite + non-sticky), NO m13 global
gentling, NO m14/m15 full-passthrough-never-inflate as the Easy strategy, NO shallow_small broad route, NO
error-guard change, NO task-id/benchmark rules, NO LLM/OpenRouter calls. The harvest engine, Hard/Medium
routing, rich/deep behavior, flip handling, and aggressive harvest for larger contexts are BYTE-IDENTICAL to
m12 — the rescue fires only where harvest fails to save (small contexts).
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "passthrough":
        ...
        # m16: SMALL-CONTEXT LIGHT-SAFE RESCUE. If harvest did not actually SAVE tokens (output >= raw input —
        # the small-context inflation signature; M/H harvests always save, so this never fires there), swap the
        # inflated harvest for the GENTLE rich path (light, never-drops, preserves load-bearing, allowed
        # BLOCK/Same-response markers). Adopt rich ONLY if it yields positive savings (never pass-through);
        # otherwise fall to raw as a last resort (never inflate). mode stays "harvest" (NON-STICKY: never traps a
        # growing M/H task in rich). final_token_estimate over raw_messages is the immutable baseline.
        _m16_harvest_out = final_token_estimate(result_messages)
        _m16_raw_in = final_token_estimate(raw_messages)
        if _m16_harvest_out >= _m16_raw_in:
            _g_msgs, _g_info = compress_gently(working)
            _gi_r, _gi_c = orphan_ids(working)
            _go_r, _go_c = orphan_ids(_g_msgs)
            _g_ok = bool(_g_msgs) and _go_r <= _gi_r and _go_c <= _gi_c
            if _g_ok and final_token_estimate(_g_msgs) < _m16_raw_in:
                result_messages = _g_msgs
                info = {**_g_info, "reason": _g_info.get("reason") or "gentle", "smallContextRescue": "rich_light"}
            else:
                result_messages = raw_messages
                info = {"reason": "small_context_never_inflate", "smallContextRescue": "raw_last_resort"}
