...
NP_RESULT_CAP = 28_000         # np3: cap 16k->28k. DIAGNOSIS (repetition analysis of 6 comp-108 trajectories): np2's Hard
                               # breaks are NOT from repeated content (exact-dup only ~10% — dedup is weak), they are from big
                               # UNIQUE tool-result blocks (24k-64k) that np2's 16k cap SHREDS (keeps 25-67%) -> the agent loses
                               # critical info -> Hard breaks/fail-fails (4 of 6 tasks had a unique block >16k). np2 (#2) already
                               # BEATS the king on Medium (+0.130) and ~ties Easy; the ONLY Overall gap is Hard (need only +~0.05).
                               # 28k keeps the 24k blocks FULL + the 56-64k blocks to 28k (vs 16k) -> fewer Hard over-compression
                               # breaks -> higher Hard -> Overall. Flat (idempotent -> cache-stable). TRADEOFF: more near-passthrough
                               # on typical -> more wander (platform-calibrated). ADDITIVE BET: np2 stays LIVE holding our 14.3%
                               # (Pair(E,M)+Single-M), so np3 can only gain (try for Overall), never lose our income. Mechanism
                               # otherwise identical to np1/np2 (cache-stable inherited).
...
# tests/diffs/paths but DROPS short structural CODE lines (imports/decorators/raise/except) -> on code reads
# the agent loses the file API -> NameError/ImportError -> BREAK. Pin them so the code skeleton survives.
# Pure per-message + deterministic (cache-stable); informative-only (no steering/markers). [np3 flat-28k + this pin = np3+pin]
