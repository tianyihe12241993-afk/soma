#!/usr/bin/env python3
"""SOMA miner m17 — m12 (m7_compliant) with the HARVEST made content-selective.

m17 CHANGE vs m12 (single variable): compress_structurally's intra-result reduction
was blind head/tail truncation; it is now the SAME extractive selection the rich
path (compress_gently) already uses — pinning error / failing-test / diff / file-path
/ code-signature lines and filling the rest by TF-IDF, at the SAME per-result byte
budget (token target / compression ratio unchanged). Rationale: the harvest already
protected error-bearing results at WHOLE-result granularity, but cut blindly WITHIN
a result, so a load-bearing clue in the middle could be lost → a break. Selection
uses active=frozenset() so it is a pure function of message text (cache-stable; no
turn-over-turn prefix churn — see EXP-1b cache finding). Assistant truncation is
unchanged. Everything else (routing, dedup, drops, escalation, markers, loop guard)
is byte-for-byte m12. Compliant: reuses the already-allowed extractive layer; no LLM
call, no task-id logic, no behavior steering. HOLD-gated: ship only if same-window
comp-108 A/B shows fewer breaks AND M/H not regressed AND net-positive after the gate.
...
def compress_structurally(
    ...
    # m17: harvest intra-result reduction is now EXTRACTIVE (same engine the rich
    # path already uses), not blind head/tail. At the SAME per-result byte budget
    # we pin error/test/diff/path/signature lines and fill the rest by TF-IDF, so
    # a load-bearing clue in the MIDDLE of a result survives instead of being cut.
    # active=frozenset() keeps selection a pure function of the message text (no
    # sliding-window dependence) → byte-stable old prefix → provider cache stays
    # warm (avoids the EXP-1b cache-invalidation cost). extractive_compress falls
    # back to head/tail truncation when extraction doesn't help, so worst case ==
    # the prior m12 behavior. Budgets map to head+tail so the token target (ratio)
    # is unchanged. Compliant: reuses the already-allowed extractive layer; no LLM,
    # no task-id, no steering, same CMP/BLOCK markers.
    harvest_active = frozenset()

    def _select_then_cap(msg: Any, ex_budget: int, cap_head: int, cap_tail: int) -> tuple[Any, bool]:
        # m17: extractive SELECTION (keeps error/test/diff/path/sig lines buried in
        # the middle) followed by a hard head/tail CAP. Line-structured results
        # shrink under the cap via extraction → the cap is a no-op → clues kept.
        # Few-line / single-line blobs that extraction cannot shrink (JSON, minified,
        # one long log line) fall through to the SAME truncate m12 used → byte budget
        # (compression ratio) is guaranteed ≤ m12 in every case. Cache-stable
        # (active=frozenset() → selection independent of the sliding window).
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult":
            ...
            if index in truncated_protected:
                message, did = _select_then_cap(
                    message, GENTLE_PROTECTED_CAP, int(GENTLE_PROTECTED_CAP * 0.75), int(GENTLE_PROTECTED_CAP * 0.25)
                )
                info["truncatedResultCount"] += int(did)
            elif index in truncated:
                message, did = _select_then_cap(message, head + tail, head, tail)
                info["truncatedResultCount"] += int(did)
            elif index in tail_results and index not in untouchable and tail_cap:
                message, _ = _select_then_cap(
                    message, TAIL_RESULT_CAP, int(TAIL_RESULT_CAP * 0.75), int(TAIL_RESULT_CAP * 0.25)
                )
            elif index not in untouchable:
                message, _ = _select_then_cap(
                    message, ABS_RESULT_CAP, int(ABS_RESULT_CAP * 0.75), int(ABS_RESULT_CAP * 0.25)
                )
            output.append(message)
            continue
        if role == "assistant":
