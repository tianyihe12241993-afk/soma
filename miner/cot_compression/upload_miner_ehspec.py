...
# ehspec: INVERTED SIZE-TIER cap = E+H SPECIALIST (targets Pair(E,H); Medium SACRIFICED/excluded).
# Small/medium results -> aggressive EH_SMALL_CAP (= m26's 6k -> Easy savings); big results -> EH_BIG_CAP
# (= np3's 28k -> keep context that FLIPS baseline-fail Hard tasks). Combines m26-Easy + np3-Hard in ONE
# miner. Hard (baseline-fail) tasks have ~1.8x bigger results than Easy -> the size boundary separates
# them (Medium ~= Hard size, but Medium is excluded from Pair(E,H) so cratering it is FREE). Both tier
# caps are PROVEN-SAFE (m26 6k / np3 28k) -> no new wander. PURE fn of this result's own size.
EH_SMALL_TIER = 16_000   # results <= this -> aggressive small cap; bigger results -> the keep cap
EH_SMALL_CAP  = 6_000    # aggressive (= m26) on small/medium results -> Easy savings
EH_BIG_CAP    = 28_000   # keep (= np3) on big results -> Hard flips


def cap_tool_result(message, cap):
    """Cache-stable per-message INVERTED SIZE-TIER cap (E+H specialist). A small/medium tool result is
    capped hard at EH_SMALL_CAP (Easy savings); a big result is capped loosely at EH_BIG_CAP (keep the
    context that flips baseline-fail Hard tasks). eff_cap is a PURE fn of this result's own size, so the
    decision is DETERMINISTIC + IDEMPOTENT (already-[[CMP]]/already-small returned UNCHANGED) -> the
    emitted prefix is byte-identical turn-to-turn -> provider cache stays warm. Medium is sacrificed
    (excluded from Pair(E,H)). `cap` arg is vestigial (eff_cap is tier-derived). Non-tool-results pass through."""
    if not isinstance(message, dict):
        return message, False
    if normalize_role(message.get("role")) != "toolResult":
        return message, False
    text = extract_text(message.get("content"))
    eff_cap = EH_SMALL_CAP if len(text) <= EH_SMALL_TIER else EH_BIG_CAP   # inverted size-tier
