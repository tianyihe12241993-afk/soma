...
TARGET_TOKENS = 12_000         # v7: gentler tight target — on hard tasks (baseline fails),
                               # PASSING is +4 vs <=+1 for savings, so preserve context for
                               # the flip; the governor still caps runaway long sessions.
PRESSURE_RELIEF = 0.75

# Harvest mode (3k-25k observed): truncation-only, but aggressive — old tool
# output is re-billed every call, so trimming it hard is the main token win.
GENTLE_RESULT_CAP = 800        # old IRRELEVANT tool results: trimmed hardest
GENTLE_RELEVANT_CAP = 4_000    # old results about a file/test the agent is ACTIVELY working: keep fuller
GENTLE_PROTECTED_CAP = 5_000   # error/test output: protected but still tightened
# v7: relevance window — paths/tests mentioned in the most recent N messages are
# the agent's current focus; stale results about those stay fuller, everything
# else is gutted. Higher signal density → more flips, fewer wanders, more savings.
RELEVANCE_WINDOW = 10
...
def _basenames(paths: set[str]) -> set[str]:
    out: set[str] = set()
    for p in paths:
        p = p.split(":")[0]  # strip :linenumber
        out.add(p)
        out.add(p.rsplit("/", 1)[-1])  # file basename
    return {b for b in out if len(b) >= 4}  # drop noise like "a.py"? keep >=4 chars


def active_paths(messages: list[Any], window: int = RELEVANCE_WINDOW) -> set[str]:
    """File paths the agent is currently focused on: those mentioned in the most
    recent `window` messages (its latest tool calls, reasoning, and errors)."""
    paths: set[str] = set()
    for message in messages[-window:]:
        if not isinstance(message, dict):
            continue
        text = extract_text(message.get("content"))
        # include tool-call arguments (where file targets usually live)
        for block in iter_tool_call_blocks(message):
...
def mentions_active(message: Any, active: set[str]) -> bool:
    if not active:
        return False
    text = extract_text(message.get("content"))[:20_000]
    found = _basenames(set(PATH_PATTERN.findall(text)))
    return bool(found & active)
...
def compress_gently(messages: list[Any]) -> tuple[list[Any], dict[str, Any]]:
    ...
    active = active_paths(messages)  # v7: files/tests the agent is currently working
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult" and index not in untouchable:
            if index in duplicate_indices:
                ...
                # v7 relevance-based retention: error output protected highest,
                # then results about an actively-worked file kept fuller, then
                # everything else (stale/irrelevant bulk) gutted hardest.
                if is_error_bearing(message):
                    cap = GENTLE_PROTECTED_CAP
                elif mentions_active(message, active):
                    cap = GENTLE_RELEVANT_CAP
                    info["relevantResultCount"] = info.get("relevantResultCount", 0) + 1
                else:
                    cap = GENTLE_RESULT_CAP
