...
def compress_gently(messages: list[Any]) -> tuple[list[Any], dict[str, Any]]:
    ...
    # m20: SUPERSEDED file-view detection. A file-view of a path that is viewed AGAIN
    # in a LATER result is redundant (the later view is the current one) -> collapse it
    # HARD (RICH_SUPERSEDED_CAP), freeing budget for a higher per-task ratio bonus on
    # PASSING tasks (score = base + 0.5*ln(ratio)). Pure-redundancy removal, so no
    # information the agent still needs is lost: the LATEST view of each path is kept
    # full, and any error/test/diff content is NEVER collapsed (_has_critical_content).
    last_path_idx: dict[str, int] = {}
    path_of: dict[int, str] = {}
    for idx in result_indices:
        p = _result_dominant_path(messages[idx])
        if p:
            path_of[idx] = p
            last_path_idx[p] = idx  # ascending iteration -> final value is the latest view
    superseded = {
        idx for idx in result_indices
        if idx not in intact
        and idx in path_of
        and last_path_idx[path_of[idx]] > idx           # a LATER view of the same path exists
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult" and index not in intact:
            if index in dup_ref:
                ...
                # m20: superseded file-view -> collapse HARD (the latest view of this
                # path, kept elsewhere, is current). Extractive keeps any pinned
                # signatures/paths; allowed CMP/"…" markers only.
                message, did = extractive_message(message, RICH_SUPERSEDED_CAP, active)
