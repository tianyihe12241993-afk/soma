...
# v18 FLIP (near-pass-through) caps. The king barely compresses flip tasks
# (comp_ratio 0.85, keeps ~85% of context) → flips 53% vs our 41% at the leaner
# 0.52. So flip-likely tasks (persistent test failures) get these much higher
# caps: keep almost everything, dedup/near-dup only, only the most pathological
# bulk trimmed. Maximises flip-critical context → more conversions. Regular rich
# (deep/cumulative, hard-both-pass) stays at the 6k caps to avoid over-context wander.
FLIP_STALE_CAP = 12_000        # 2x rich's 6k — keep more, target king's ~0.85 (not near-pass-through)
FLIP_PROTECTED_CAP = 40_000    # load-bearing kept fuller than rich's 30k
FLIP_INTACT_MSGS = 14          # larger byte-intact recent window than rich's 10
...
# v15 PERSISTENT-FAILURE routing trigger. Platform data showed flip tasks (e.g.
# 250/255) over-compressed in HARVEST and broke (-2.3) because the strict
# error-guard (last-4-all-error) missed them — their REAL test failures were
# interspersed with reads, so the window never went all-error. Fix: count REAL
# repeated TEST/ASSERTION failures across the whole trajectory (NOT generic
# command errors), and once a task has failed tests in PERSIST_FAIL_THRESHOLD
# distinct rounds (a flip-task signature: baseline-hard, keeps failing), route it
# to the SAME proven 6k-rich path EARLIER — before the agent commits a wrong fix.
# Medium tasks resolve in 1-2 test-fail cycles (< threshold) so they STAY harvest
# (bonus preserved). Rich PAYLOAD is unchanged (6k/30k/10) — this is purely a
# routing change. TEST failures only — generic "error:"/"failed"/"fatal:" excluded.
TEST_FAILURE_PATTERN = re.compile(
    r"\b[1-9]\d*\s+failed\b"            # pytest "N failed" (N>=1, not "0 failed")
    r"|\b[1-9]\d*\s+errors?\b"          # "N errors" (N>=1)
    r"|\bFAILED\b"                      # pytest FAILED markers
    r"|\bAssertionError\b"              # assertion failures
    r"|^\s*(?:FAIL|ERROR):\s",          # unittest "FAIL:"/"ERROR:" lines
    re.M,
)
PERSIST_FAIL_THRESHOLD = 3     # v16: 4->3, catch flip-tasks one round SOONER (more flips, route before wrong-fix)
# v26 DEPTH-GATED flip threshold. Catch flips at the earlier threshold ONLY when the
# task is deep/large (flip-likely); shallow tasks (medium/easy) keep threshold-3 to
# avoid v25's flip-mode-wander drag. The real middle-ground between v24(3) and v25(2).
V26_DEEP_THRESHOLD = 2         # earlier catch for deep/flip-likely tasks
V26_DEEP_OBS = 60_000          # current context >= this -> deep/flip-likely
V26_DEEP_CUM = 300_000         # cumulative work >= this -> deep
V26_DEEP_MSGS = 60             # message depth >= this -> deep
PERSIST_FAIL_WINDOW = 4        # scan this many most-recent tool results for THIS round's test failure
# v22 RELEASE-FLIP-ON-PASS. flip-mode (near-pass-through 0.85) is meant for true
# break-flips (baseline-fail, hard, tests NEVER pass). But the persistent-failure
# trigger also fires on hard BOTH-PASS tasks that iterate through failures (e.g.
# django-14007 -> 0.87 comp_ratio = wasted bonus on a task that passes anyway).
# Fix: once tests CLEARLY pass, the task is resolved (a both-pass) -> set a sticky
# flag and revert flip->rich/harvest to recover the comp-ratio bonus on the
# verification tail. A real break-flip never passes -> stays in flip (0.85). High
# precision required: a false pass-signal would drop a break-flip to lean and
# break it, so we require a clear pass marker AND no failure marker in the result.
TEST_PASS_PATTERN = re.compile(
    r"^=+[^\n]*\b[1-9]\d*\s+passed\b[^\n]*=+\s*$"   # pytest summary "=== N passed ... ==="
    r"|\b[1-9]\d*\s+passed\b(?![^\n]*\bfailed\b)"   # "N passed" with no "failed" on the line
    r"|^OK\s*$"                                     # unittest "OK" on its own line
    r"|^OK\s*\([^)]*\)\s*$",                        # unittest "OK (skipped=...)"
    re.M,
)
...
def save_state(
    ...
    *,
    ...
    test_fail_rounds: int = 0,
    ever_test_pass: bool = False,
    ...
    state = {
        ...
        "testFailRounds": int(test_fail_rounds),
        "everTestPass": bool(ever_test_pass),
...
def resolve_stateful_messages(
    ...
    extras = {
        ...
        "testFailRounds": int(state.get("testFailRounds") or 0) if state else 0,
        "everTestPass": bool(state.get("everTestPass")) if state else False,
...
def _latest_result_clean_pass(messages: list[Any]) -> bool:
    """v24: most-recent tool result shows a CLEAR test pass (clean pass marker AND
    no failure marker). High precision — a flip/unsolved task never cleanly passes,
    so this fires only when the task is genuinely done."""
    for m in reversed(messages):
        if isinstance(m, dict) and normalize_role(m.get("role")) == "toolResult":
            t = extract_text(m.get("content"))[:30_000]
            return (not TEST_FAILURE_PATTERN.search(t)) and bool(TEST_PASS_PATTERN.search(t))
    return False
...
def build_coach_text(messages: list[Any], *, governor: bool) -> str:
    ...
    # Forced stopping at a CLEAR condition (organizer-permitted). v24: when tests
    # are ACTUALLY observed passing, fire a FIRM stop — this is the documented
    # decisiveness lever (cut the post-fix verification-tail wander: the agent
    # makes the fix early, then burns 20-30 turns re-verifying). Detection is
    # high-precision (clean pass, no failure) so it never fires on an unsolved
    # task; a passing task is done, so stopping cannot reduce its score. Falls
    # back to the generic note when no pass is observed.
    if _latest_result_clean_pass(messages):
        parts.append(
            "Your tests are passing — the change is complete. STOP NOW: return your "
            "final answer. Do not create more tests, re-verify, or keep exploring."
        )
    else:
        parts.append(
...
def compress_gently(
    messages: list[Any],
    stale_cap: int = RICH_STALE_CAP,
    protected_cap: int = RICH_PROTECTED_CAP,
    intact_msgs: int = RICH_INTACT_MSGS,
) -> tuple[list[Any], dict[str, Any]]:
    # v10 RICH+RELIABLE+HARVEST compression. NEVER drops a message and never
    # touches the recent working window or any load-bearing result. Caps are
    # parameterized (v18): flip-likely tasks pass NEAR-PASS-THROUGH caps (keep
    # ~85% like the king's flip strategy) while regular rich uses the 6k caps.
    info: dict[str, Any] = {
    ...
    # Last intact_msgs messages stay byte-intact (the agent's working set).
    intact = set(range(max(0, len(messages) - intact_msgs), len(messages)))
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult" and index not in intact:
            if index in duplicate_indices:
                ...
                message, did = extractive_message(message, protected_cap, active)
                ...
                message, did = extractive_message(message, stale_cap, active)
                info["truncatedResultCount"] += int(did)
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # v15 persistent-failure routing: count rounds where the agent saw a REAL
    # test/assertion failure (NOT generic command errors). Monotonic counter in
    # state. Once a task has failed tests in PERSIST_FAIL_THRESHOLD distinct
    # rounds (flip-task signature: baseline-hard, keeps failing), route it to the
    # SAME 6k-rich path EARLY — before it over-compresses into a wrong fix (250/255).
    # Count ONLY when the SINGLE most-recent tool result is a test failure — i.e.
    # the task is *still* failing tests THIS round. Scanning a window would keep
    # counting old failures that linger in the recent messages even after a task
    # RESOLVES, which would wrongly route resolving medium tasks to rich (and kill
    # the Medium bonus). Checking just the newest result means a task that goes
    # green stops incrementing → stays harvest.
    round_test_fail = False
    round_test_pass = False
    for m in reversed(raw_messages):
        if isinstance(m, dict) and normalize_role(m.get("role")) == "toolResult":
            _rt = extract_text(m.get("content"))[:30_000]
            round_test_fail = bool(TEST_FAILURE_PATTERN.search(_rt))
            # v22: a CLEAR pass (clear pass marker AND no failure marker this round).
            round_test_pass = (not round_test_fail) and bool(TEST_PASS_PATTERN.search(_rt))
            break
    test_fail_rounds = int(extras.get("testFailRounds") or 0) + (1 if round_test_fail else 0)
    # v26: DEPTH-GATED threshold. Pure threshold-2 (v25) caught flips earlier but
    # dragged shallow medium/easy into flip-mode wander (11099 41c/0.97). Fix: use
    # the earlier threshold (2) ONLY for deep/large (flip-likely) tasks — where the
    # wrong-fix-commit break happens and richer context helps — and keep the safe
    # threshold-3 for shallow tasks (medium/easy), so they don't get pulled in early.
    # Deep = big current context OR lots of cumulative work OR deep message count.
    deep_flip = (observed >= V26_DEEP_OBS or cumulative_observed >= V26_DEEP_CUM
                 or msg_depth >= V26_DEEP_MSGS)
    eff_threshold = V26_DEEP_THRESHOLD if deep_flip else PERSIST_FAIL_THRESHOLD
    persistent_failure = test_fail_rounds >= eff_threshold
    # v22: sticky "tests have clearly passed at some point" flag. A true break-flip
    # (baseline-fail, hard) never passes → ever_test_pass stays False → it stays in
    # flip-mode (0.85). A hard BOTH-PASS that tripped the flip trigger while
    # iterating WILL pass → ever_test_pass becomes True → flip is released back to
    # the rich/harvest path, recovering the comp-ratio bonus on the verification tail.
    ever_test_pass = bool(extras.get("everTestPass")) or round_test_pass
    # v18: FLIP-likely tasks (persistently failing tests / still-failing) get the
    # near-pass-through FLIP caps (keep ~85%, the king's flip playbook → more
    # conversions). Other large tasks (deep/cumulative, hard-both-pass that
    # resolved) get the regular 6k rich. Monotonic+sticky: ...->rich->flip.
    # v22: ...but RELEASE flip once tests clearly passed (it was a both-pass).
    if (prev_mode == "flip" or persistent_failure or still_failing) and not ever_test_pass:
        mode = "flip"
    elif (
        prev_mode == "rich"
    ...
    if mode == "passthrough":
        ...
        # Rich (6k caps) OR flip (near-pass-through caps, ~0.85 — king's flip
        # playbook). Both truncation-only via compress_gently; flip keeps ~85%.
        sanitized = working
        if mode == "flip":
            result_messages, info = compress_gently(
                working, FLIP_STALE_CAP, FLIP_PROTECTED_CAP, FLIP_INTACT_MSGS
            )
        else:
    ...
    metadata = {
        ...
        **info,
        ...
        "testFailRounds": test_fail_rounds,
        "persistentFailure": persistent_failure,
        "everTestPass": ever_test_pass,
        "flipReleased": bool((persistent_failure or still_failing or prev_mode == "flip") and ever_test_pass),
        "msgDepth": msg_depth,
    ...
    try:
        save_state(
            ...
            test_fail_rounds=test_fail_rounds,
            ever_test_pass=ever_test_pass,
