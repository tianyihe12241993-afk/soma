#!/usr/bin/env python3
"""SOMA miner uphard_salience_CAP64 (64k cap + salience import-pin, cache-stable) — derived from salience/m12.
...
NP_MAX_KEEP = 64_000           # CAP64 VARIANT (middle): a HUGE result keeps min(0.60*len, 64k) -> keeps MORE of nocap's upside
                               # than cap32 (results 53-107k kept fuller) while still bounding the EXTREME uncapped tail at 64k
                               # (a 500k result -> 64k, not nocap's 300k). + salience import-pin. Differs from salience(cap32) &
                               # nocap ONLY in this constant. Balanced ceiling/risk. Net map: <=16k pass(=np2) | 16-26.7k cap16k(=np2) |
                               # 26.7-107k keep 0.60 (fuller than cap32) | >107k cap 64k.
