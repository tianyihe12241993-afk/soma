#!/usr/bin/env python3
"""SOMA miner m14_never_inflate_only — m12 (upload_miner_m7_compliant.py) + EXACTLY ONE change.

THE ONLY change vs m12: a NEVER-INFLATE GUARD. After compression (harvest OR rich), if the final output's
token estimate is >= the raw input's (we did not actually save tokens — ratio would be < 1.0), discard the
compressed form and emit PASS-THROUGH (raw messages, changed=False). Guarantees ratio >= 1.0 — the miner can
never grow the context. This is the m13 change that PLATFORM-helped Easy (0.412->0.561); m14 isolates it.

DELIBERATELY EXCLUDED (m13's other changes that the platform showed COLLAPSED Hard 0.919->0.434 + Medium
0.953->0.718): NO aggressiveness cap (MAX_COMPRESSION_RATIO), NO harvest>8x->rich fallback, NO repeated-failure
routing, NO shallow/Easy pass-through routing, NO error-guard change, NO determinism rewrite, NO task-specific
logic. The harvest engine, Hard/Medium routing, rich fallback, aggressive harvest, load-bearing protections,
tool-call/tool-result pairing, and the allowed-markers-only compliance are BYTE-IDENTICAL to m12 — the
guard only fires when compression would otherwise inflate (output >= input), which on normal compressible
Medium/Hard contexts never happens, so those stay exactly as m12.
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # -----------------------------------------------------------------------
    # m14: NEVER-INFLATE GUARD (the ONLY change vs m12) — final step before return,
    ...
    # guarantees ratio >= 1.0 — the miner can never grow the context. On normal
    # compressible Medium/Hard contexts the output is < raw, so this never fires and
    # the result is byte-identical to m12.
