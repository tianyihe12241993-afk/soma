#!/usr/bin/env python3
"""SOMA miner H4b_compliant_h1m_deep — RULE-COMPLIANT derivative of h1m@deep (m7/v11.1 + profile).

WHAT THIS IS: the h1m@deep compression ENGINE kept intact, with every prompt-side
behavior-steering construct removed so the miner complies with
miner/README_prompting.md. h1m@deep = the m7/v11 depth-adaptive engine with the
HARVEST path tightened by the baked "deep" profile (TARGET_TOKENS 5200 + tighter
progressive truncation caps) — the profile that gate-ACCEPTed in eval (+28%
compression, Medium 8/8). The README permits exactly two prompt-edit categories
next round:
...
PROFILE BAKED: H1M_PROFILE is hardcoded to "deep" (the compression container does
NOT receive the env var, so the deep profile must be baked in). The _H1M_PROFILES
dict + the deep params are kept; only the resolution line is hardcoded.

REMOVED vs h1m@deep (all prompt-side, now illegal):
  - The entire injected coach message and the force-stop governor that told the
    ...
    changing steering, the failing-test restatement / issue-reinjection). These
    steered the agent's workflow and are now gone.
  - The first-user-message HISTORY DIGEST injection. h1m@deep's harvest path appended
    a growing one-line digest of dropped interactions, wrapped in a private marker,
  ...
  - All custom/private bracketed markers h1m emitted (the compressed-history digest
...
KEPT vs h1m@deep (the compression ENGINE, unchanged): depth-adaptive
passthrough/harvest/rich routing, the baked deep HARVEST structural drop+truncate
path (compress_structurally), the RICH skeleton (compress_gently), the fragile/error
...
exactly as h1m@deep did — the allowed markers merely WRAP it.

Protocol (same as the reference): `python upload_miner_h4b_compliant.py assemble`
...
cli_main / the plugin entry contract are identical to m7/h1m so the existing driver
...
# ===========================================================================
# H1M PROFILE LAYER — the ONLY compression-knob change vs m7/v11.1.
# Overrides harvest-path knobs only (TARGET + progressive truncation caps).
# RICH_* / GENTLE_PROTECTED_CAP / ERROR_MARKERS / load-bearing preservation are
# untouched, so protected content and the rich/flip path stay exactly as m7. The
# error-guard fires one hit sooner so borderline still-failing tasks bail to rich
# (= m7). The profile is BAKED to "deep" — the compression container does NOT
# receive the H1M_PROFILE env var, so the deep profile (the one that gate-ACCEPTed
# in eval) must be hardcoded here. The _H1M_PROFILES dict is kept for provenance.
_H1M_PROFILES = {
...
H1M_PROFILE = "deep"  # BAKED: env var does not reach the compression container.
(TARGET_TOKENS, MID_HEAD, MID_TAIL, MID2_HEAD, MID2_TAIL,
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    #   harvest    : shallow -> v6 drop+truncate (deep target 5200) -> pass-task bonus.
    ...
    if mode == "passthrough":
        ...
        # v6-style aggressive drop+truncate (deep profile, target 5200): compress the
        # trajectory by truncating old tool output (wrapped in the allowed [[CMP]]…
        # [[/CMP]] markers) and dropping the oldest interactions, recovering the
        # pass-task token bonus on shallow tasks. The frozen head (first user message)
        # stays byte-identical — NO history-digest injection. Orphan-guarded; never
        # ships pairing we broke ourselves.
        sanitized = working
