...
# v14: the REAL king (1.335) wins on HARD (1.525) not Easy; we beat it on Medium
# (1.421 vs 1.197). So the lever is HARD/flips, and we ENRICH the rich path only
# (which fires on hard/still-failing tasks via the error-guard) → more flips +
# fewer confident-wrong-fix breaks → Hard climbs toward 1.525. Harvest is left
# UNTOUCHED so Medium stays at 1.421 (our advantage). Richer rich = more tokens
# on hard tasks (the king runs flips at 1-2.9M, so this is their proven shape).
RICH_INTACT_MSGS = 14          # v14: 10->14 — larger byte-intact recent window for hard fixes
RICH_STALE_CAP = 12_000        # v14: 6k->12k — keep old non-load-bearing bulk richer for flip context
RICH_PROTECTED_CAP = 50_000    # v14: 30k->50k — load-bearing (errors/tests/active/edits) kept fuller
