...
# v10 RICH+RELIABLE+HARVEST: v9's reliability skeleton (recent window byte-intact,
# everything load-bearing kept FULL — errors, tests, active files, recent edits)
# is unchanged, because that is what produced the zero-break profile. The only
# change vs v9 is on the EXPENDABLE half: old, non-load-bearing, stale bulk is
# trimmed HARDER (cap 6k -> 2k chars) and near-duplicate old results (same content
# modulo line numbers / counts / timestamps) collapse to a marker like exact dups
# already do. This harvests the easy/medium token bonus (TokA down -> score up)
# while leaving every byte the agent actually needs intact, so hard flips and the
# zero-break profile are preserved. Validated against v9 by the 12-run harness;
# ship v10 only if it keeps zero breaks AND improves savings, else ship v9.
RICH_INTACT_MSGS = 10          # last N messages kept byte-intact (never compressed)
RICH_STALE_CAP = 2_000         # v10: aggressive cap for old, non-protected, stale bulk (was 6k)
