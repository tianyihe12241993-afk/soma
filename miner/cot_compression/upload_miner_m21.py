#!/usr/bin/env python3
"""SOMA miner m21 — "lighter harvest" derivative of m12 (upload_miner_m7_compliant).

WHAT THIS IS vs m12: m12's engine kept BYTE-IDENTICAL on the RICH (compress_gently),
PASSTHROUGH, ROUTER (handle_assemble mode-decision + all routing constants), and
ORPHAN-GUARD paths. The ONLY changes are scoped to the HARVEST path
(compress_structurally + the HARVEST-only constants), which is reached only when the
router selects mode=="harvest". Thesis: keep MORE dropped whole-interactions in the
harvest path (paying only the minor 0.5*ln ratio cost) so shallow harvest-routed
break-runs (e.g. whole-interaction-drop breaks) are reduced, WITHOUT touching the
RICH path that Hard tasks route to — so Hard 0.919 is protected by construction.

The four scoped HARVEST-only changes (see CHANGE 1..4 inline):
  1) keep-more knob: TARGET_TOKENS 8k->11k, PRESSURE_RELIEF 0.75->1.0,
     TIGHT_KEEP_RECENT 4->6 (~25% global savings, still +5pp over the 20% floor,
     multiplier-saturated).
  2) a never-drop retention class (is_retention_critical): source-edit / test-spec
     interactions are protected from the drop loop entirely (they still TRUNCATE,
     bounded by every cap) — the fix m17 could not do (m17 only re-picked WHAT
     survived truncation; it could not recover DROPPED interactions).
  3) a pure never-inflate-on-RAW-tokens guard: the m21 harvest output is never
     larger (in raw chars) than the m12-equivalent build would be; on inflation it
     falls back to the m12-equivalent build for that turn. Makes a multiplier
     collapse impossible; worst case == m12.
  4) extractive-then-cap on what IS still truncated (ported from m17's
     _select_then_cap): keep error/test/diff/path/signature lines buried in a
     truncated result at the SAME byte budget; worst case == m12 head/tail.

HARD-SAFETY: all four changes are structurally unreachable by Hard (Hard routes to
RICH, which is byte-identical). No new mode is added (the m18 ultralight mode was the
fatal vector); no new "lighten this turn" gate is keyed on depth/observed/cumulative/
prev_mode/escalation. The lightening only changes HOW the existing harvest path
budgets and drops — the same turns m12 harvested are harvested, just less aggressively.

INHERITED FROM m12 (the rule-compliance posture, unchanged): every prompt-side
...
# m21: bump from m12's 4 -> 5. The harvest path produces different output bytes than
# m12, so a state file written by one must not be read by the other (load_state
# rejects a mismatched version). Router/passthrough/rich logic is unchanged.
STATE_VERSION = 5
...
# m21 CHANGE 1 (HARVEST-ONLY keep-more knob; read only by compress_structurally,
# reached only via mode=="harvest"). m12: TARGET_TOKENS=8_000, PRESSURE_RELIEF=0.75.
# Raising the target + setting relief to 1.0 keeps MORE oldest interactions
# (fewer whole-interaction drops) at ~25% global savings — still +5pp over the 20%
# floor, multiplier-saturated (the global savings multiplier is flat at 1.0 well
# above the floor, so this costs only the minor 0.5*ln ratio term, not the
# multiplier). cache is irrelevant to the score (RAW tokens), so PRESSURE_RELIEF's
# only rationale — a byte-stable prefix for provider cache — is dead; setting it to
# 1.0 stops the drop loop as soon as the target is met instead of over-pruning to
# 0.75x target. RICH/passthrough/router are untouched (see HARD-SAFETY).
TARGET_TOKENS = 11_000         # m21: 8_000 -> 11_000 (lighter harvest)
PRESSURE_RELIEF = 1.0          # m21: 0.75 -> 1.0 (stop the past-target over-prune)
# m21 CHANGE 3: never-inflate / savings-floor guard (HARVEST-ONLY). Purpose (per the
# spec): protect the GLOBAL savings floor and make a multiplier collapse impossible.
#
# NOTE on the spec's literal wording vs its intent. The spec text says "never contain
# MORE raw tokens than m12's harvest output" (INV-4). Taken literally that nullifies
# the WHOLE design: CHANGE 1+2 deliberately KEEP MORE (the calibration targets ~25%
# global savings vs m12's 37.8% — i.e. m21 is INTENTIONALLY larger than m12 on every
# turn it helps), so a strict "<= m12 bytes" guard would fall back to m12 on exactly
# those turns and m21 would be byte-equivalent to m12 everywhere. The spec's STATED
# PURPOSE — "protects the GLOBAL savings floor ... makes a multiplier collapse
# impossible" — is the controlling intent. So the guard enforces a per-turn SAVINGS
# FLOOR (output keeps at least HARVEST_MIN_SAVINGS of the raw input compressed away),
# NOT a "<= m12 bytes" rule. This lets m21 keep more (the design) while guaranteeing
# the savings multiplier can never collapse below the floor. On a floor breach we fall
# back to the m12-equivalent build for that turn (worst case == m12). This is a
# deliberate reconciliation of a self-contradictory spec; see the implementer report.
HARVEST_NEVER_INFLATE = True
# 20% is the named multiplier floor; m21's GLOBAL target is ~25% (+5pp margin). This
# per-turn floor is conservative (a single rich-kept turn rarely dominates the GLOBAL
# average), and on breach the turn reverts to the m12-equivalent build, so the global
# savings multiplier is structurally protected from collapse.
HARVEST_MIN_SAVINGS = 0.20
# m21 CHANGE 1: legacy (m12) harvest constants, used ONLY by the never-inflate guard
# to recompute the m12-equivalent plan/build for the comparison + fallback. The live
# harvest path uses the lighter TARGET_TOKENS / PRESSURE_RELIEF / TIGHT_KEEP_RECENT
# above; these mirror m12 exactly so the fallback is byte-equivalent to m12.
LEGACY_TARGET_TOKENS = 8_000
LEGACY_PRESSURE_RELIEF = 0.75
LEGACY_TIGHT_KEEP_RECENT = 4
...
# m21 CHANGE 1 (HARVEST-ONLY): widen the recent-intact tail from 4 to 6 so the 5-6
# most recent interactions survive un-dropped (the edit/test history on shallow
# tasks lives here). Bounded so it cannot starve the budget. GENTLE_* (rich path)
# unchanged.
TIGHT_KEEP_RECENT = 6          # m21: 4 -> 6 (keep more recent interactions intact)
TIGHT_UNTOUCHABLE = 2
...
# m21 CHANGE 2: source-edit / test-spec interactions are the fix-establishing
# history. The harvest drop loop must sacrifice only true boilerplate (duplicate
# reads, dir listings, find/grep/pip/poll chatter), never these. This is a pure
# function of message text — no task id, no category logic, no steering, no LLM:
# a deterministic content class. m17 only re-picked WHAT survived truncation; it
# could not recover DROPPED interactions. Protecting these from the drop loop is
# the actual fix for whole-interaction-drop breaks (they still TRUNCATE, bounded by
# every cap — they cost bounded bytes, not whole-trajectory budget).
_SOURCE_EDIT_RE = re.compile(
    r'"?(?:tool_?name|name)"?\s*[:=]\s*"?(edit|write|str_replace|create)\b', re.I
)
_TEST_PATH_RE = re.compile(r'\btests?/[\w./-]+\.py\b|::test_|def test_', re.I)


def is_retention_critical(call_msg: Any, result_msg: Any) -> bool:
    """True if the invoking tool CALL names an edit/write to source, OR the CALL
    targets a test file / test function. Scanned on the assistant CALL block text +
    the call's name/arguments/input ONLY — never on the result text.

    m21 fixer note (over-match repair): an earlier build also scanned the RESULT text
    with _TEST_PATH_RE, which fired on any bulky read/cat/grep/pytest-collection
    output that merely MENTIONED a test path (extremely common). That retained ~all
    such results, collapsing the pass/pass token bonus (measured: 40/40 plain reads
    retained -> 0.77 savings vs m12's 0.90) on exactly the easy/medium harvest-routed
    tasks m21 is meant to help. Retention now keys on the CALL only — the agent's
    intent to edit source / target a test — which is the genuine fix-establishing
    signal m17 could not recover, while leaving incidental test-path mentions in
    boilerplate read output fully droppable. result_msg is unused (kept in the
    signature so the call site is unchanged)."""
    del result_msg  # intentionally not inspected — retention is a CALL-only signal
    call_text = extract_text(call_msg.get("content")) if isinstance(call_msg, dict) else ""
    # Tool-call arguments often live in dedicated fields, not the text content, so
    # also scan the call blocks' name + arguments/args/input (where the edited /
    # targeted path lives). This is the call's intent, not the result's content.
    if isinstance(call_msg, dict):
        for block in iter_tool_call_blocks(call_msg):
            name = block.get("name") or block.get("toolName") or ""
            call_text += "\nname:" + str(name)
            for field in ("arguments", "args", "input"):
                if field in block:
                    call_text += "\n" + extract_text(block[field])
    if _SOURCE_EDIT_RE.search(call_text):
        return True
    if _TEST_PATH_RE.search(call_text):
        return True
    return False
...
def _harvest_build(
    ...
    *,
    ...
    keep_recent: int,
    use_retention: bool,
    use_select_then_cap: bool,
) -> tuple[list[Any], dict[str, Any]]:
    """The m12 harvest engine, parameterized so m21 can run it two ways:
      - the LIGHT (m21) build: target 11k, relief 1.0, keep_recent 6, retention on,
        extractive-then-cap on;
      - the LEGACY (m12-equivalent) build: target 8k, relief 0.75, keep_recent 4,
        retention off, plain truncate — used only by the never-inflate guard for the
        comparison + fallback (CHANGE 3), so the worst case is exactly m12.
    `escalation` is mutated in place (the caller passes a copy for the build it does
    not commit). All four m21 changes are flag-gated here; with use_retention=False
    and use_select_then_cap=False and the legacy knob values, this is byte-identical
    to m12's compress_structurally."""
    info: dict[str, Any] = {
    ...
    # m21 CHANGE 1: with pressure_relief=1.0 the drop loop stops as soon as the
    # target is met (no past-target over-prune); the legacy build passes 0.75.
    relief_chars = int(target_chars * pressure_relief)

    # Tail zone: the invoking call of the keep_recent-th newest result onward.
    # m21 CHANGE 1: keep_recent is 6 in the light build (was/legacy 4).
    recent_results = result_indices[-keep_recent:]
    ...
    for result_index in reversed([i for i in result_indices if i < tail_start]):
        ...
        # m21 CHANGE 2: a never-drop retention class. The interaction is retention-
        # critical if its invoking CALL is an edit/write to source, or the CALL
        # targets a test file/function (CALL-only signal — the result text is NOT
        # scanned, so incidental test-path mentions in bulky read output stay
        # droppable). Off in the legacy build (use_retention=False) so the fallback
        # is byte-identical to m12.
        call_msg = messages[call_index] if call_index is not None else None
        retain = (
            use_retention and is_retention_critical(call_msg, messages[result_index])
        )
        interactions.append(
            {
                ...
                # m21 CHANGE 2: retained interactions are never converted to drop.
                "retain": retain,
                "mode": "trunc",
            }
        )
    ...
    def planned_chars(assist_cap: bool, trunc2: bool, tail_cap: bool) -> int:
        total = 0
        dropped_count = 0
        tail_result_set = set(result_indices[-keep_recent:])
    ...
    # m21 CHANGE 2: never drop a retention-critical interaction in the duplicate
    # pre-drop (it still truncates below).
    for item in interactions:
        if item["duplicate"] and item["droppable"] and not item["retain"]:
            item["mode"] = "drop"
    ...
    if planned_chars(assist_cap, trunc2, tail_cap) > target_chars:
        # Sacrifice boilerplate first, then protected (error/test) output, and
        # retained (source-edit/test) interactions last/never. m21 CHANGE 2: the
        # sort key leads with `retain` so retained items sort last, and the drop is
        # skipped for them entirely; the budget is freed from duplicate reads, dir
        # listings, and find/grep/pip/poll chatter — where the recoverable bytes are.
        for item in sorted(
            interactions, key=lambda it: (it["retain"], it["protected"], it["result_index"])
        ):
            if planned_chars(assist_cap, trunc2, tail_cap) <= relief_chars:
                break
            if item["droppable"] and not item["retain"]:
                item["mode"] = "drop"
    if planned_chars(assist_cap, trunc2, tail_cap) > target_chars:
        trunc2 = True
    if planned_chars(assist_cap, trunc2, tail_cap) > target_chars:
    ...
    untouchable = set(result_indices[-TIGHT_UNTOUCHABLE:])
    tail_results = set(result_indices[-keep_recent:])

    # m21 CHANGE 4: intra-result reduction is EXTRACTIVE-then-cap (ported from m17)
    # in the light build. At the SAME per-result byte budget it pins error/test/diff/
    # path/signature lines buried in the middle of a result, so a load-bearing clue
    # survives instead of being cut. active=frozenset() keeps selection a pure
    # function of the message text (cache-stable). extractive_compress falls back to
    # head/tail when extraction does not shrink, so worst case == m12 truncation.
    # In the legacy build (use_select_then_cap=False) this is the plain m12
    # truncate_message, so the fallback is byte-identical to m12.
    harvest_active = frozenset()

    def _reduce(msg: Any, ex_budget: int, cap_head: int, cap_tail: int) -> tuple[Any, bool]:
        if use_select_then_cap:
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult":
            ...
            if index in truncated_protected:
                ...
                message, did = _reduce(message, head + tail, head, tail)
                info["truncatedResultCount"] += int(did)
...
def compress_structurally(
    ...
    """m21 harvest entry point. Runs the LIGHT (keep-more) build, then applies the
    savings-floor guard (CHANGE 3): the light build may keep MORE than m12 (the
    design), but if its per-turn savings vs the raw input fall below HARVEST_MIN_
    SAVINGS, fall back to the m12-equivalent build for this turn so the global savings
    multiplier can never collapse. Cache is irrelevant to the score (RAW tokens), so
    savings are measured on raw chars. Worst case on a floor breach is exactly m12."""
    escalation = escalation if escalation is not None else {}

    # m21 light build. Operate on a copy of escalation so a fallback does not commit
    # light-build escalation flags into the persisted state.
    light_esc = dict(escalation)
    light_output, light_info = _harvest_build(
        messages,
        light_esc,
        target_tokens,
        pressure_relief=PRESSURE_RELIEF,
        keep_recent=TIGHT_KEEP_RECENT,
        use_retention=True,
        use_select_then_cap=True,
    )

    if not HARVEST_NEVER_INFLATE:
        escalation.clear()
        escalation.update(light_esc)
        return light_output, light_info

    input_chars = sum(message_cost(m) for m in messages)
    light_chars = sum(message_cost(m) for m in light_output)
    # Per-turn savings of the light build vs the raw input. If the input is tiny or
    # un-compressible, savings may legitimately be ~0 and there is nothing to protect
    # (max guards a zero-division and means a 0-char input never triggers fallback).
    light_savings = (input_chars - light_chars) / max(1, input_chars)

    if light_savings >= HARVEST_MIN_SAVINGS:
        # Light build clears the savings floor → ship it (this is the keep-more win).
        light_info["neverInflate"] = {
            "inputChars": input_chars,
            "lightChars": light_chars,
            "lightSavings": round(light_savings, 4),
            "floor": HARVEST_MIN_SAVINGS,
            "fellBack": False,
        }
        escalation.clear()
        escalation.update(light_esc)
    ...
    # Floor breach: the light build kept so much that per-turn savings fell below the
    # floor. Fall back to the m12-equivalent build for this turn (worst case == m12).
    legacy_esc = dict(escalation)
    legacy_output, legacy_info = _harvest_build(
        messages,
        legacy_esc,
        LEGACY_TARGET_TOKENS,
        pressure_relief=LEGACY_PRESSURE_RELIEF,
        keep_recent=LEGACY_TIGHT_KEEP_RECENT,
        use_retention=False,
        use_select_then_cap=False,
    )
    legacy_chars = sum(message_cost(m) for m in legacy_output)
    legacy_info["reason"] = "savings_floor_fallback_m12"
    legacy_info["neverInflate"] = {
        "inputChars": input_chars,
        "lightChars": light_chars,
        "lightSavings": round(light_savings, 4),
