"""SOMA comp-110 (CoT-Compression-5) — INTERIOR-CAP f1km (floor 1k, STRONG-signal-only salience -> fewer markers).
...
_SALIENT_RX = re.compile(
    r"""(?x)
      (?: \bFile\s+"[^"]+",\s+line\s+\d+ )
    | (?: \b(?:Traceback|Error|Exception|error\[|FAILED|PASSED|ERROR|assert(?:ion)?\b) )
    | (?: \btests?[_/][\w./-]+ | \btest_\w+ )
    | (?: ^\s*diff\s | ^\s*---\s | ^\s*\+\+\+\s )
    | (?: [\w\-./]+\.(?:py|c|h|cpp|js|ts|go|rs|java|rb|json|yaml|yml|toml|cfg|txt|rst|md)\b )
    """,
    re.M,
)
...
def _cap_interior(text: str) -> str:
    ...
    for i in range(n):                          # verbatim HEAD run (whole lines) up to FLOOR chars
        ...
        keep[i] = True
        used += cost
    for i in range(n):                          # ALL salient lines survive verbatim
