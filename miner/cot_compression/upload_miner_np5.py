...
NP_RESULT_CAP = 48_000         # np5 = NEAR-PURE-PASSTHROUGH Overall-CHALLENGER (Bet 1). np2=16k / np3=28k; np5=48k = the
                               # near-passthrough cap region we NEVER tested = exactly the shape #1 (5DCnA57, 0.697) wins
                               # Overall with. RATIONALE (report §15-E + §18): #1 beats us on Overall PURELY by run-stability
                               # (fewer breaks: 9 vs our 11-14), NOT compression/quality/provider (all ~equal). Our miners all
                               # extractive-CMP big results (byte-change -> agent-path divergence -> breaks); a much lighter cap
                               # TOUCHES FAR FEWER results -> minimal divergence -> fewer breaks -> #1's profile. KEY: Easy tasks
                               # have SMALL results (<16k), so np5 treats them IDENTICALLY to np2 -> inherits np2's HIGH Easy
                               # (~1.05), NOT np3's anomalous 0.859 (that gap was Easy-draw VARIANCE, not the cap). np5 only
                               # diverges from np2 on BIG (Medium/Hard) results, which it keeps fuller -> fewer over-compression
                               # breaks (+Hard) at a slight Medium-ratio cost. NET BET: high Easy (=np2) + fewer breaks (light) +
                               # higher Hard -> Overall toward/past #1's 0.697 (np2 is 0.684, gap only 0.013). Keeps a SLIVER of
                               # compression (trims the 48-64k monsters) for a ratio EDGE over #1's pure passthrough (our clean-run
                               # mean 1.206 > #1's 1.157, §15-E). ADDITIVE: wins NO corner (near-passthrough = balanced) so it can't
                               # cannibalize np2(E+M)/np3(M+H) = 23.8%; it's a dedicated Overall ticket. Flat (idempotent ->
                               # cache-stable). 48k = first cut; PLATFORM calibrates (lighter 56-64k = fewer breaks/less ratio;
                               # heavier 40k = more ratio/more breaks). Mechanism otherwise identical to np1/np2 (cache-stable inherited).
