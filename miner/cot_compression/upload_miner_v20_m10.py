...
# v20 ACTIVE-FILE SNAPSHOT. Trajectory research (django-14007, 48-step wander)
# showed the dominant wander is RE-READING files during exploration: the agent
# read base.py/query.py/compiler.py 3-4 times each because earlier reads were
# stale-trimmed away. Fix: keep the LATEST read of each re-read file at the
# protected cap even when stale, so the agent never needs to re-fetch a file it
# already saw. Strictly additive — only upgrades a stale read to protected; never
# drops content (older/near-dup reads still collapse via the existing dedup pass).
READ_REVISIT_THRESHOLD = 2     # a file read this many times -> snapshot its latest read full
...
def _call_path_map(messages: list[Any]) -> dict[str, str]:
    """v20: map tool-call id -> the single file path referenced in that call's args.
    A call referencing exactly one file path is treated as a 'file view' of that
    path; its result can then be keyed to the file. Calls with zero or multiple
    paths are skipped (ambiguous)."""
    out: dict[str, str] = {}
    for message in messages:
        for block in iter_tool_call_blocks(message):
            cid = (block.get("id") or "").strip()
            if not cid:
                continue
            args = extract_text(
                block.get("arguments") or block.get("args") or block.get("input") or ""
            )
            paths = {p.split(":")[0] for p in PATH_PATTERN.findall(args)}
            if len(paths) == 1:
                out[cid] = next(iter(paths))
    return out


def _snapshot_indices(messages: list[Any], result_indices: list[int], intact: set[int]) -> set[int]:
    """v20: indices of the LATEST read of each file that was read >=
    READ_REVISIT_THRESHOLD times (and is now stale, i.e. not in the intact window).
    These get protected-cap treatment so the agent doesn't re-read them."""
    call_path = _call_path_map(messages)
    read_count: dict[str, int] = {}
    latest_idx: dict[str, int] = {}
    for index in result_indices:
        path = None
        for rid in extract_tool_result_ids(messages[index]):
            if rid in call_path:
                path = call_path[rid]
                break
        if path:
            read_count[path] = read_count.get(path, 0) + 1
            latest_idx[path] = index  # result_indices ascending -> ends as latest
    return {
        idx for path, idx in latest_idx.items()
        if read_count[path] >= READ_REVISIT_THRESHOLD and idx not in intact
    }
...
def compress_gently(
    ...
    # v20: latest read of each re-read file -> keep full (prevents re-reads).
    snapshot_indices = _snapshot_indices(messages, result_indices, intact)
    ...
    for index, message in enumerate(messages):
        ...
        if role == "toolResult" and index not in intact:
            ...
            elif index in snapshot_indices or _is_load_bearing(message, active):
                # keep full (guard only against pathological size). v20: the latest
                # read of a re-read file is snapshot-protected here even when stale,
                # so the agent doesn't re-fetch a file it already saw.
                message, did = extractive_message(message, protected_cap, active)
                info["protectedResultCount"] += 1
                info["snapshotResultCount"] += int(index in snapshot_indices)
...
def handle_assemble(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for m in reversed(raw_messages):
        if isinstance(m, dict) and normalize_role(m.get("role")) == "toolResult":
            round_test_fail = bool(TEST_FAILURE_PATTERN.search(extract_text(m.get("content"))[:30_000]))
            break
    ...
    if prev_mode == "flip" or persistent_failure or still_failing:
        mode = "flip"
    elif (
        prev_mode == "rich"
